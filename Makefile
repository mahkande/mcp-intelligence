...
# https://github.com/bobmatnyc/mcp-code-intelligence
...
PROJECT_NAME := mcp-code-intelligence
PYTHON_VERSION := 3.11
...
homebrew-test: ## Test Homebrew Formula locally
	...
	@echo "  brew tap bobmatnyc/mcp-code-intelligence"
	@echo "  brew install --build-from-source mcp-code-intelligence"
...
debug-search: ## Debug search with logging (usage: make debug-search QUERY="term")
	...
	LOGURU_LEVEL=DEBUG $(UV) run mcp-code-intelligence search "$(QUERY)" --verbose
...
debug-mcp: ## Debug MCP server with logging
	...
	LOGURU_LEVEL=DEBUG $(UV) run mcp-code-intelligence mcp --debug

.PHONY: debug-status
debug-status: ## Debug project health status
	@echo "$(GREEN)Checking project health...$(NC)"
	$(UV) run mcp-code-intelligence status --verbose --debug
...
debug-index-status: ## Debug index status and health
	...
	$(UV) run mcp-code-intelligence status --verbose
	@echo "Checking for .mcp-code-intelligence directory..."
	ls -la .mcp-code-intelligence/ 2>/dev/null || echo "No project initialized"
...
debug-performance: ## Debug search performance
	...
	$(UV) run python -c "import time; start=time.time(); from mcp_code_intelligence.core import search; print(f'Import time: {time.time()-start:.3f}s')"
...
test-mcp: ## Test MCP server integration
	...
	timeout 5s $(UV) run mcp-code-intelligence mcp || echo "MCP server test completed"
...
integration-test: build-package ## Run integration tests with built package
	...
	mcp-code-intelligence init --file-extensions .py --embedding-model sentence-transformers/all-MiniLM-L6-v2; \
	mcp-code-intelligence index; \
	mcp-code-intelligence search "function" --limit 5; \
...
verify-setup: ## Verify installation and setup
	...
	$(UV) run python -c "import mcp_code_intelligence; print('✓ Package imports successfully')"
