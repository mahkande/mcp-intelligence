...
from mcp_code_intelligence.cli.commands.install import (
    detect_project_root,
    find_git_root,
)
...
class TestProjectRootDetection:
    ...
    def test_detect_with_mcp_code_intelligence_directory(self, tmp_path: Path):
        ...
        mcp_dir = tmp_path / ".mcp-code-intelligence"
        mcp_dir.mkdir()
    ...
    def test_detect_with_git_and_mcp_directory(self, tmp_path: Path):
        """Should detect git root when .mcp-code-intelligence exists there."""
        # Create git repo structure
        git_root = tmp_path / "project"
        git_root.mkdir()
        (git_root / ".git").mkdir()
        (git_root / ".mcp-code-intelligence").mkdir()
    ...
    def test_detect_fallback_to_current_directory(self, tmp_path: Path):
        ...
        # No .git or .mcp-code-intelligence
        result = detect_project_root(tmp_path)
        assert result == tmp_path

    def test_detect_prefers_mcp_directory_over_git(self, tmp_path: Path):
        """Should prefer .mcp-code-intelligence in current dir over git root."""
        # Create git repo
        git_root = tmp_path / "git-project"
        git_root.mkdir()
        (git_root / ".git").mkdir()

        # Create subdirectory with .mcp-code-intelligence
        subdir = git_root / "sub-project"
        subdir.mkdir()
        (subdir / ".mcp-code-intelligence").mkdir()
...
class TestEndToEndScenarios:
    ...
    def test_scenario_fresh_project_installation(self, tmp_path: Path):
        ...
        # Setup: User has a git repo but no .mcp-code-intelligence yet
    ...
    def test_scenario_nested_subdirectory_installation(self, tmp_path: Path):
        ...
        project.mkdir()
        (project / ".git").mkdir()
        (project / ".mcp-code-intelligence").mkdir()
    ...
    def test_scenario_monorepo_with_multiple_projects(self, tmp_path: Path):
        ...
        # Project A has its own mcp-code-intelligence
        project_a = monorepo / "project-a"
        project_a.mkdir()
        (project_a / ".mcp-code-intelligence").mkdir()

        # Project B has its own mcp-code-intelligence
        project_b = monorepo / "project-b"
        project_b.mkdir()
        (project_b / ".mcp-code-intelligence").mkdir()
    ...
    def test_scenario_non_git_project(self, tmp_path: Path):
        ...
        # Should still detect via .mcp-code-intelligence
        detected = detect_project_root(project)
        assert detected == project
