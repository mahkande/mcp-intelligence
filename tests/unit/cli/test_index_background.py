...
from mcp_code_intelligence.cli.commands.index import (
...
class TestSpawnBackgroundIndexer:
    ...
    @patch("mcp_code_intelligence.cli.commands.index.subprocess.Popen")
    @patch("mcp_code_intelligence.cli.commands.index._is_process_alive")
    def test_spawn_background_indexer_success_unix(
        ...
        cmd = call_args[0][0]
        assert "-m" in cmd
        assert "mcp_code_intelligence.cli.commands.index_background" in cmd
    ...
    def test_spawn_background_indexer_already_running(self, mock_is_alive, tmp_path):
        ...
        with patch(
            "mcp_code_intelligence.cli.commands.index.subprocess.Popen"
        ) as mock_popen:
...
class TestIsProcessAlive:
    ...
    def test_is_process_alive_windows_alive(self):
        ...
        with (
            patch("mcp_code_intelligence.cli.commands.index.sys.platform", "win32"),
...
class TestCancelBackgroundIndexer:
    ...
    @patch("mcp_code_intelligence.cli.commands.index.os.kill")
    @patch("mcp_code_intelligence.cli.commands.index._is_process_alive")
    def test_cancel_unix_success(self, mock_is_alive, mock_kill, tmp_path):
        ...
        # Test
        with (
            patch("mcp_code_intelligence.cli.commands.index.sys.platform", "linux"),
            patch("time.sleep"),
        ):
            _cancel_background_indexer(project_root, force=True)
