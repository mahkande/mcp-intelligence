...
from mcp_code_intelligence.cli.commands.chat import run_chat_analyze
...
class TestRunChatAnalyze:
    ...
    async def test_analyze_basic_query(self, mock_project_root, mock_llm_client):
        ...
        with (
            ...
            patch(
                "mcp_code_intelligence.core.project.ProjectManager"
            ) as mock_project_manager,
            patch(
                "mcp_code_intelligence.core.config_utils.get_openai_api_key"
            ) as mock_openai_key,
            patch(
                "mcp_code_intelligence.core.config_utils.get_openrouter_api_key"
            ) as mock_openrouter_key,
            patch(
                "mcp_code_intelligence.parsers.registry.ParserRegistry"
            ) as mock_parser_registry,
            patch("mcp_code_intelligence.analysis.ProjectMetrics") as mock_project_metrics,
            patch(
                "mcp_code_intelligence.analysis.interpretation.EnhancedJSONExporter"
            ) as mock_exporter,
        ):
            # Setup mocks
    ...
    async def test_analyze_fallback_on_stream_error(
        ...
        with (
            ...
            patch(
                "mcp_code_intelligence.analysis.interpretation.AnalysisInterpreter"
            ) as mock_interpreter,
        ):
            # Setup mocks
            mock_openai_key.return_value = "test-key"
