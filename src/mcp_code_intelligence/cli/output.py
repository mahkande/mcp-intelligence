...
from mcp_code_intelligence.core.models import ProjectInfo, SearchResult
...
def print_search_results(
    ...
    for i, result in enumerate(results, 1):
        ...
        if quality_weight > 0.0 and hasattr(result, "_original_similarity"):
            ...
            # Pure semantic search: show similarity score with optional breakdown
            if result.base_score is not None and result.boost_score is not None:
                final_score = result.similarity_score
                base = result.base_score
                boost_val = result.boost_score
                
                similarity = (
                    f"[green]{final_score:.2%}[/green] "
                    f"[dim](Jina: {base:.2%}, Boosts: {boost_val:+.2f})[/dim]"
                )
            else:
...
def print_error_with_recovery(error_message: str, recovery_steps: list[str]) -> None:
    ...
    console.print("\n[dim]For more help: mcp-code-intelligence --help[/dim]")
