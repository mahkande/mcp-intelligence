"""Search command for MCP Code Intelligence CLI."""
...
from mcp_code_intelligence.cli.output import (
    print_error,
    print_info,
...
# This makes `mcp-code-intelligence search "query"` work as main search
# and `mcp-code-intelligence search SUBCOMMAND` work for subcommands
...
def search_main(
    ...
    [green]Simple semantic search:[/green]
        $ mcp-code-intelligence search "authentication middleware"

    [green]Search with language filter:[/green]
        $ mcp-code-intelligence search "database connection" --language python

    [green]Limit results:[/green]
        $ mcp-code-intelligence search "error handling" --limit 5

    [bold cyan]Advanced Search:[/bold cyan]

    [green]Filter by file pattern (glob):[/green]
        $ mcp-code-intelligence search "validation" --files "*.py"
        $ mcp-code-intelligence search "component" --files "src/*.tsx"
        $ mcp-code-intelligence search "test utils" --files "tests/*.ts"

    [green]Find similar code:[/green]
        $ mcp-code-intelligence search "src/auth.py" --similar

    [green]Context-based search:[/green]
        $ mcp-code-intelligence search "implement rate limiting" --context --focus security

    [bold cyan]Quality Filters:[/bold cyan]

    [green]Filter by complexity:[/green]
        $ mcp-code-intelligence search "authentication" --max-complexity 15

    [green]Exclude code smells:[/green]
        $ mcp-code-intelligence search "login" --no-smells

    [green]Filter by grade:[/green]
        $ mcp-code-intelligence search "api" --grade A,B

    [green]Minimum quality score:[/green]
        $ mcp-code-intelligence search "handler" --min-quality 80

    [green]Quality-aware ranking:[/green]
        $ mcp-code-intelligence search "auth" --quality-weight 0.5  # Balance relevance and quality
        $ mcp-code-intelligence search "api" --quality-weight 0.0   # Pure semantic search
        $ mcp-code-intelligence search "util" --quality-weight 1.0  # Pure quality ranking

    [bold cyan]Export Results:[/bold cyan]

    [green]Export to JSON:[/green]
        $ mcp-code-intelligence search "api endpoints" --export json

    [green]Export to markdown:[/green]
        $ mcp-code-intelligence search "utils" --export markdown
...
async def run_search(
    ...
    try:
        async with database:
            ...
            if export_format:
                from mcp_code_intelligence.cli.export import SearchResultExporter, get_export_path
                ...
                if export_format == "json":
                    ...
                    from mcp_code_intelligence.cli.output import print_error
            ...
            from mcp_code_intelligence.cli.history import SearchHistory
            ...
            # Display results
            if json_output:
                from mcp_code_intelligence.cli.output import print_json
                ...
                if results:
                    ...
                    print_info(
                        "  • Check if files are indexed with [cyan]mcp-code-intelligence status[/cyan]"
                    )
...
def search_similar_cmd(
    ...
    Examples:
        mcp-code-intelligence search similar src/auth.py
        mcp-code-intelligence search similar src/utils.py --function validate_email
    """
    try:
        project_root = project_root or ctx.obj.get("project_root") or Path.cwd()
...
def search_context_cmd(
    ...
    Examples:
        mcp-code-intelligence search context "implement rate limiting"
        mcp-code-intelligence search context "user authentication" --focus security,middleware
    """
    try:
        project_root = project_root or ctx.obj.get("project_root") or Path.cwd()
...
def interactive_search(
    ...
    Examples:
        ...
        mcp-code-intelligence search interactive --project-root /path/to/project
    """
    import asyncio

    from mcp_code_intelligence.cli.interactive import start_interactive_search
    from mcp_code_intelligence.cli.output import console
...
def show_history(
    ...
    Examples:
        ...
        mcp-code-intelligence search history --limit 50
    """
    from mcp_code_intelligence.cli.history import show_search_history
...
def show_favorites_cmd(
    ...
    Examples:
        mcp-code-intelligence search favorites                # List all favorites
        mcp-code-intelligence search favorites list           # List all favorites
        mcp-code-intelligence search favorites add "auth"     # Add favorite
        mcp-code-intelligence search favorites remove "auth"  # Remove favorite
    """
    from mcp_code_intelligence.cli.history import SearchHistory, show_favorites
...
# Add main command to search_app (allows: mcp-code-intelligence search main "query")
