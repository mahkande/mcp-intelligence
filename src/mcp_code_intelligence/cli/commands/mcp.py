...
Configure mcp-code-intelligence as an MCP server for various AI coding assistants.
...
[bold cyan]Quick Start:[/bold cyan]
  1. List tools:     [green]mcp-code-intelligence mcp list[/green]
  2. Configure tool: [green]mcp-code-intelligence mcp <tool>[/green]
  3. Test setup:     [green]mcp-code-intelligence mcp test[/green]
...
def mcp_callback(ctx: typer.Context):
    ...
    from mcp_code_intelligence.mcp_impl.server import run_mcp_server
...
def get_mcp_server_command(
    ...
    return f"{python_exe} -m mcp_code_intelligence.mcp_impl.server{watch_flag} {project_root}"


def detect_install_method() -> tuple[str, list[str]]:
    """Detect how mcp-code-intelligence is installed and return appropriate command.

    Returns:
        Tuple of (command, args) for running mcp-code-intelligence mcp
    """
    ...
    if os.environ.get("VIRTUAL_ENV") and ".venv" in os.environ.get("VIRTUAL_ENV", ""):
        ...
        if shutil.which("uv"):
            return ("uv", ["run", "mcp-code-intelligence", "mcp"])

    # Check if mcp-code-intelligence is directly available in PATH
    mcp_cmd = shutil.which("mcp-code-intelligence")
    if mcp_cmd:
        # Installed via pipx or pip - use direct command
        return ("mcp-code-intelligence", ["mcp"])
...
def configure_tool_mcp(
    tool_name: str,
    project_root: Path,
    server_name: str = "mcp-code-intelligence",
    enable_file_watching: bool = True,
    force: bool = False,
) -> bool:
...
def configure_auggie(
    ...
    Sets up mcp-code-intelligence as an MCP server for Auggie AI assistant.
    Configuration is stored in ~/.augment/settings.json.

    [bold cyan]Examples:[/bold cyan]

    [green]Configure with defaults:[/green]
        $ mcp-code-intelligence mcp auggie

    [green]Force overwrite existing config:[/green]
        $ mcp-code-intelligence mcp auggie --force

    [green]Disable file watching:[/green]
        $ mcp-code-intelligence mcp auggie --no-watch
    """
    try:
...
def configure_claude_code(
    ...
    [green]Configure with defaults:[/green]
        $ mcp-code-intelligence mcp claude-code

    [green]Force overwrite existing config:[/green]
        $ mcp-code-intelligence mcp claude-code --force

    [green]Disable file watching:[/green]
        $ mcp-code-intelligence mcp claude-code --no-watch
    """
    try:
        project_root = ctx.obj.get("project_root") or Path.cwd()
        project_manager = ProjectManager(project_root)
...
def configure_codex(
    ...
    Sets up mcp-code-intelligence as an MCP server for OpenAI Codex CLI.
    Configuration is stored in ~/.codex/config.toml.

    [bold cyan]Examples:[/bold cyan]

    [green]Configure with defaults:[/green]
        $ mcp-code-intelligence mcp codex

    [green]Force overwrite existing config:[/green]
        $ mcp-code-intelligence mcp codex --force

    [green]Disable file watching:[/green]
        $ mcp-code-intelligence mcp codex --no-watch
    """
    try:
        project_root = ctx.obj.get("project_root") or Path.cwd()
        project_manager = ProjectManager(project_root)
...
def configure_gemini(
    ...
    Sets up mcp-code-intelligence as an MCP server for Google Gemini CLI.
    Configuration is stored in ~/.gemini/mcp.json.

    [bold cyan]Examples:[/bold cyan]

    [green]Configure with defaults:[/green]
        $ mcp-code-intelligence mcp gemini

    [green]Force overwrite existing config:[/green]
        $ mcp-code-intelligence mcp gemini --force

    [green]Disable file watching:[/green]
        $ mcp-code-intelligence mcp gemini --no-watch
    """
    try:
        project_root = ctx.obj.get("project_root") or Path.cwd()
        project_manager = ProjectManager(project_root)
...
@mcp_app.command("roo-code")
def configure_roo_code(
    ctx: typer.Context,
    server_name: str = typer.Option(
        "mcp-code-intelligence",
        "--name",
    ...
    try:
        ...
        print_error(f"Configuration failed: {e}")
        raise typer.Exit(1)
@mcp_app.command("vscode")
@mcp_app.command("copilot")
def configure_vscode(
    ...
    """🤖 Configure MCP integration for VS Code / GitHub Copilot.

    Creates .vscode/mcp.json (official standard) and updates .vscode/settings.json.
    
    [bold cyan]Examples:[/bold cyan]

    [green]Configure:[/green]
        $ mcp-code-intelligence mcp vscode
    """
    try:
        from .setup.mcp_config import MCPConfigManager
        from .setup.main import run_setup_workflow
        ...
        # This mirrors the logic in setup/main.py
        ...
        server_cfg = {
            server_name: {
                "type": "stdio",
                "command": python_cmd,
                "args": ["-u", "-m", "mcp_code_intelligence.mcp_impl.server"],
                "env": {
        ...
        print_info(f"🔗 Configuring VS Code MCP for project: {project_root.name}")
        
        # 1. Write official .vscode/mcp.json
        mcp_man.write_local_config(server_cfg)
        
        # 2. Update .vscode/settings.json (for Copilot fallback)
        mcp_man.inject_vscode_settings(server_cfg)
        
        # 3. Create instruction files
        mcp_man.inject_copilot_instructions(server_cfg)
        mcp_man.inject_universal_rules(server_cfg)
        
        print_success("🎉 VS Code / Copilot integration configured successfully!")
        print_info("💡 Please restart VS Code or Reload Window to see the tools.")
        print_info("💡 Ensure you have the 'pre-release' version of GitHub Copilot extension.")
...
@mcp_app.command("cursor")
def configure_cursor_alias(ctx: typer.Context, force: bool = False) -> None:
    """🤖 Alias for roo-code / VS Code configuration for Cursor."""
    configure_roo_code(ctx, force=force)
...
def install_mcp_integration(
    ...
    print_info("  • mcp-code-intelligence mcp auggie")
    print_info("  • mcp-code-intelligence mcp claude-code")
    print_info("  • mcp-code-intelligence mcp codex")
    print_info("  • mcp-code-intelligence mcp gemini")
    print_info("")
    ...
    try:
        ...
        print_info("\nTo test the integration, run: mcp-code-intelligence mcp test")

    except ProjectNotFoundError:
        print_error(f"Project not initialized at {project_root}")
        print_info("Run 'mcp-code-intelligence init' in the project directory first")
        raise typer.Exit(1)
    except Exception as e:
...
def list_tools() -> None:
    ...
    they are currently configured with mcp-code-intelligence.

    [bold cyan]Examples:[/bold cyan]

    [green]List all tools:[/green]
        $ mcp-code-intelligence mcp list
    """
        ...
        try:
            if config_path.exists():
                if tool_info["format"] == "json":
                    with open(config_path) as f:
                        config = json.load(f)
                    if config.get("mcpServers", {}).get("mcp-code-intelligence"):
                        status = "✅ Configured"
                    else:
                    ...
                    with open(config_path, "rb") as f:
                        config = tomllib.load(f)
                    if config.get("mcp_servers", {}).get("mcp-code-intelligence"):
                        status = "✅ Configured"
                    else:
                        status = "⚠️  Config exists"
            else:
    ...
    console.print(table)
    console.print(
        "\n[dim]💡 Use 'mcp-code-intelligence mcp <tool>' to configure a specific tool[/dim]"
    )
...
def test_mcp_integration(
    ...
    [green]Test default server:[/green]
        $ mcp-code-intelligence mcp test

    [green]Test custom server:[/green]
        $ mcp-code-intelligence mcp test --name my-search-server
    ...
    try:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                print_info(
                    "Run 'mcp-code-intelligence mcp install' or 'mcp-code-intelligence mcp init' first"
                )
                raise typer.Exit(1)
...
def remove_mcp_integration(
    ctx: typer.Context,
    server_name: str = typer.Option(
        "mcp-code-intelligence", "--name", help="Name of the MCP server to remove"
    ),
...
def show_mcp_status(
    ...
    [green]Check integration status:[/green]
        $ mcp-code-intelligence mcp status

    [green]Check specific server:[/green]
        $ mcp-code-intelligence mcp status --name my-search-server
