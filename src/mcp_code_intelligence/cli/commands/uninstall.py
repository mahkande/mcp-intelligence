...
Examples:
    ...
    # Use alias
    $ mcp-code-intelligence remove mcp
"""
...
[bold cyan]Usage Patterns:[/bold cyan]
  ...
  [green]1. Remove from Auto-Detected Platform[/green]
     ...
     [code]$ mcp-code-intelligence uninstall mcp[/code]

  [green]2. Remove from Specific Platform[/green]
     Remove from a specific platform:
     [code]$ mcp-code-intelligence uninstall mcp --platform cursor[/code]

  [green]3. Remove from All Platforms[/green]
     Remove from all configured platforms:
     [code]$ mcp-code-intelligence uninstall mcp --all[/code]

  [green]4. List Current Installations[/green]
     See what's currently configured:
     [code]$ mcp-code-intelligence uninstall list[/code]
...
[dim]💡 Alias: 'mcp-code-intelligence remove' works the same way[/dim]
""",
...
def find_configured_platforms() -> list[PlatformInfo]:
    """Find all platforms that have mcp-code-intelligence configured.

    Returns:
        List of platforms with mcp-code-intelligence installed
    """
...
def uninstall_from_platform(platform_info: PlatformInfo) -> bool:
    ...
    try:
        ...
        # Uninstall server
        result = installer.uninstall_server("mcp-code-intelligence")
...
def uninstall_mcp(
    ...
    [dim]💡 Use 'mcp-code-intelligence uninstall list' to see configured platforms[/dim]
    """
    ...
    try:
        ...
        if platform:
            ...
            if not target_platforms:
                print_error(
                    f"Platform '{platform}' does not have mcp-code-intelligence configured"
                )
                raise typer.Exit(1)
...
def list_integrations(ctx: typer.Context) -> None:
    ...
    try:
        ...
        if not configured:
            ...
            console.print(
                "\n[dim]Use 'mcp-code-intelligence install mcp' to add integrations[/dim]"
            )
            return
        ...
        for platform_info in configured:
            table.add_row(
                ...
                f"mcp-code-intelligence uninstall mcp --platform {platform_info.platform.value}",
            )
        ...
        console.print(
            "  • Remove specific: [code]mcp-code-intelligence uninstall mcp --platform <name>[/code]"
        )
        console.print(
            "  • Remove all:      [code]mcp-code-intelligence uninstall mcp --all[/code]"
        )
