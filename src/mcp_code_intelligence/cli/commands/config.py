"""Config command for MCP Code Intelligence CLI."""
...
from mcp_code_intelligence.core.exceptions import ConfigurationError, ProjectNotFoundError
...
def set(
    ...
    Examples:
        ...
        mcp-code-intelligence config set embedding_model microsoft/unixcoder-base
        mcp-code-intelligence config set cache_embeddings true
    """
    try:
        project_root = project_root or ctx.obj.get("project_root") or Path.cwd()
        project_manager = ProjectManager(project_root)
...
def _parse_config_value(key: str, value: str):
    ...
    # Float values
    if key in ["similarity_threshold", "throttle_delay"]:
        try:
            parsed = float(value)
    ...
    # Integer values
    if key in ["max_chunk_size", "max_cache_size", "max_workers", "max_file_size_kb"]:
        try:
            parsed = int(value)
            if parsed <= 0:
...
def _show_available_keys() -> None:
    ...
    keys_info = [
        ...
        ("max_workers", "Max worker processes for parsing", "integer"),
    ...
    console.print(
        "\n[dim]Use 'mcp-code-intelligence config set <key> <value>' to change values[/dim]"
    )
...
def list_embedding_models() -> None:
    ...
    console.print(
        "[dim]Set model: [cyan]mcp-code-intelligence config set embedding_model MODEL_NAME[/cyan][/dim]"
    )
