"""Visualization commands for MCP Code Intelligence.
...
from mcp_code_intelligence.cli.commands.visualize.graph_builder import build_graph_data
...
def export(
    ...
    Examples:
        ...
        mcp-code-intelligence visualize export --file src/main.py

        # Custom output location
        mcp-code-intelligence visualize export -o graph.json
...
async def _export_chunks(
    ...
    try:
        ...
        if len(chunks) == 0:
            console.print(
                "[yellow]No chunks found in index. Run 'mcp-code-intelligence index' first.[/yellow]"
            )
            raise typer.Exit(1)
        ...
        console.print(
            Panel.fit(
                ...
                f"[dim]Next: Run 'mcp-code-intelligence visualize serve' to view[/dim]",
...
def serve(
    ...
    Examples:
        ...
        # Custom port
        mcp-code-intelligence visualize serve --port 3000

        # Custom graph file
        mcp-code-intelligence visualize serve --graph my-graph.json

        # Serve with code-only filter
        mcp-code-intelligence visualize serve --code-only
    """
    ...
    if graph_file.exists() and not needs_regeneration:
        index_db = (
            project_manager.project_root / ".mcp-code-intelligence" / "chroma.sqlite3"
        )
        if index_db.exists():
