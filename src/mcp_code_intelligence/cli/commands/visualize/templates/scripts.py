...
function buildTreeStructure() {
    ...
    // - Directory with single directory child: src -> mcp_code_intelligence becomes "src/mcp_code_intelligence"
...
function showTrends() {
    ...
    if (window.graphTrendData && window.graphTrendData.entries && window.graphTrendData.entries.length > 0) {
        ...
        html += 'Run <code>mcp-code-intelligence index</code> to generate the first snapshot.';
        html += '</div>';
        html += '</div>';
    }
