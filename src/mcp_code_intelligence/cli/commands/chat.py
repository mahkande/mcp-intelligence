...
def chat_main(
    ...
    [green]Ask how something works:[/green]
        $ mcp-code-intelligence chat "how does the indexing process work?"

    [green]Find implementation details:[/green]
        $ mcp-code-intelligence chat "show me the search ranking algorithm"
    ...
    [green]Filter by file pattern:[/green]
        $ mcp-code-intelligence chat "how does validation work?" --files "*.py"
        $ mcp-code-intelligence chat "find React components" --files "src/*.tsx"

    [green]Limit results:[/green]
        $ mcp-code-intelligence chat "find auth code" --limit 3

    [green]Custom timeout:[/green]
        $ mcp-code-intelligence chat "complex question" --timeout 60

    [green]Use advanced model for complex queries:[/green]
        $ mcp-code-intelligence chat "explain the authentication flow" --think
...
async def run_chat_with_intent(
    ...
    config_dir = project_root / ".mcp-code-intelligence"
...
async def run_chat_answer(
    ...
    Args:
        ...
        files: File pattern filter
    """
    from mcp_code_intelligence.core.config_utils import get_openai_api_key, get_openrouter_api_key
...
async def run_chat_analyze(
    ...
    from mcp_code_intelligence.analysis.interpretation import AnalysisInterpreter, EnhancedJSONExporter
    from mcp_code_intelligence.core.config_utils import get_openai_api_key, get_openrouter_api_key
...
async def run_chat_search(
    ...
    if provider:
        ...
        if provider == "openai" and not openai_key:
            ...
            print_info("")
            print_info("Or run: [cyan]mcp-code-intelligence setup[/cyan]")
            raise typer.Exit(1)
    ...
    if total_results == 0:
        ...
        print_info(
            "  • Check if relevant files are indexed with [cyan]mcp-code-intelligence status[/cyan]"
        )
        raise typer.Exit(0)
...
async def _show_fallback_results(
    ...
    """
    from mcp_code_intelligence.cli.output import print_search_results
