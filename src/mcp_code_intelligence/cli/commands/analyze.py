"""Analyze command for MCP Code Intelligence CLI."""
...
from mcp_code_intelligence.analysis.baseline import (
    BaselineComparator,
    BaselineExistsError,
    BaselineManager,
    BaselineNotFoundError,
)
from mcp_code_intelligence.analysis.storage.metrics_store import MetricsStore, MetricsStoreError
from mcp_code_intelligence.analysis.storage.trend_tracker import TrendData, TrendDirection, TrendTracker
from mcp_code_intelligence.core.exceptions import ProjectNotFoundError
from mcp_code_intelligence.core.git import GitError, GitManager, GitNotAvailableError, GitNotRepoError
...
from mcp_code_intelligence.cli.output import console, print_error, print_info, print_json
...
def main(
    ...
    [green]Filter by language:[/green]
        $ mcp-code-intelligence analyze --language python

    [green]Analyze specific directory:[/green]
        $ mcp-code-intelligence analyze --path src/core
    ...
    [green]Show top 5 hotspots:[/green]
        $ mcp-code-intelligence analyze --top 5

    [green]Export to JSON:[/green]
        $ mcp-code-intelligence analyze --json > analysis.json

    [green]Export to SARIF format:[/green]
        $ mcp-code-intelligence analyze --format sarif --output report.sarif

    [green]Export to Markdown format:[/green]
        $ mcp-code-intelligence analyze --format markdown --output .
    ...
    [green]Fail on WARNING or ERROR smells:[/green]
        $ mcp-code-intelligence analyze --fail-on-smell --severity-threshold warning

    [green]CI/CD workflow with SARIF:[/green]
        $ mcp-code-intelligence analyze --fail-on-smell --format sarif --output report.sarif
    ...
    if list_baselines:
        ...
        if not baselines:
            ...
            console.print(
                "Create a baseline with: [cyan]mcp-code-intelligence analyze --save-baseline <name>[/cyan]"
            )
        else:
...
def filter_smells_by_severity(smells: list, severity_threshold: str) -> list:
    ...
    """
    from mcp_code_intelligence.analysis.collectors.smells import SmellSeverity
...
async def run_analysis(
    ...
    try:
        ...
        try:
            metrics_db_path = project_root / ".mcp-code-intelligence" / "metrics.db"
        ...
        all_smells = []
        if show_smells:
            from mcp_code_intelligence.analysis.collectors.smells import SmellDetector
        ...
        if output_format == "markdown":
            ...
            if include_context:
                ...
                from mcp_code_intelligence.analysis.interpretation import EnhancedJSONExporter
                from mcp_code_intelligence.config.thresholds import ThresholdConfig
...
async def _analyze_file(
    ...
    """
    try:
        # Get parser for file (run in thread to prevent blocking during init)
        # Add timeout (15s) in case parser init tries to download things and hangs
        try:
             parser = await asyncio.wait_for(
                 asyncio.to_thread(parser_registry.get_parser_for_file, file_path),
                 timeout=15.0
             )
        except asyncio.TimeoutError:
             logger.warning(f"Parser initialization timed out for {file_path}")
             return None

        # Read file content safely in thread
        def read_content_safe(p):
            try:
        ...
        try:
             content = await asyncio.wait_for(
                 asyncio.to_thread(read_content_safe, file_path),
                 timeout=5.0
             )
        except asyncio.TimeoutError:
             logger.warning(f"File reading timed out for {file_path}")
             return None
        ...
        # Parse content into chunks with timeout
        # Note: parse_content is 'async' but might contain blocking calls (tree-sitter).
        # We wrap it in wait_for, but if it blocks CPU hard, wait_for might delay.
        # Ideally we would run parse_content in thread too, but it is async def.
        try:
             chunks = await asyncio.wait_for(parser.parse_content(content, file_path), timeout=30.0)
        except asyncio.TimeoutError:
             logger.warning(f"Analysis timed out for {file_path}")
             return None
        except Exception as e:
             logger.debug(f"Parsing failed for {file_path}: {e}")
             chunks = []

        # Create file metrics
        file_metrics = FileMetrics(file_path=str(file_path))
        
        # Set language based on extension (User Request)
        ext = file_path.suffix.lower()
        if ext == ".py":
            file_metrics.language = "python"
        elif ext == ".md":
            file_metrics.language = "markdown"
        elif ext in (".js", ".jsx"):
            file_metrics.language = "javascript"
        elif ext in (".ts", ".tsx"):
            file_metrics.language = "typescript"
        elif hasattr(parser, "language"):
            file_metrics.language = parser.language

        # Count lines (we already have content)
        ...
        if chunks:
            ...
            from mcp_code_intelligence.analysis.metrics import ChunkMetrics
    
            for chunk in chunks:
                # Use complexity_score from parser
                complexity = (
