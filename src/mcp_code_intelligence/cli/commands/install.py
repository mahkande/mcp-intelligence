"""Install and integration commands for MCP Code Intelligence CLI.
...
from mcp_code_intelligence.config.defaults import DEFAULT_EMBEDDING_MODELS, DEFAULT_FILE_EXTENSIONS
...
install_app = create_enhanced_typer(
    help="""📦 Install mcp-code-intelligence and MCP integrations

[bold cyan]Usage Patterns:[/bold cyan]

  [green]1. Project Installation (Primary)[/green]
     Install mcp-code-intelligence in the current project:
     [code]$ mcp-code-intelligence install[/code]

  [green]2. MCP Platform Integration[/green]
     Add MCP integration with auto-detection:
     [code]$ mcp-code-intelligence install mcp[/code]
     [code]$ mcp-code-intelligence install mcp --platform cursor[/code]
     [code]$ mcp-code-intelligence install mcp --all[/code]

  [green]3. Complete Setup[/green]
     Install project + all MCP integrations:
     [code]$ mcp-code-intelligence install --with-mcp[/code]
...
[dim]💡 Use 'mcp-code-intelligence uninstall mcp' to remove integrations[/dim]
""",
...
def detect_project_root(start_path: Path | None = None) -> Path:
    ...
    Detection priority:
    1. Directory with .mcp-code-intelligence/ (project initialized)
    ...
    # Check for .mcp-code-intelligence directory (initialized project)
    if (current / ".mcp-code-intelligence").exists():
        logger.debug(f"Detected project root via .mcp-code-intelligence: {current}")
        return current

    # Check if we're in a git repository
    git_root = find_git_root(current)
    if git_root and (git_root / ".mcp-code-intelligence").exists():
        logger.debug(f"Detected project root via git + .mcp-code-intelligence: {git_root}")
        return git_root
...
def main(
    ...
    This command initializes mcp-code-intelligence with:
    ...
    [dim]💡 After installation, use 'mcp-code-intelligence search' to search your code[/dim]
    """
    ...
    try:
        ...
        console.print(
            Panel.fit(
                f"[bold cyan]Installing mcp-code-intelligence[/bold cyan]\n"
        ...
        if auto_index:
            ...
            try:
                ...
                print_info("   Run 'mcp-code-intelligence index' to index later")
        ...
        if with_mcp:
            ...
            if detected:
                ...
                print_info("  mcp-code-intelligence install mcp --platform <platform>")

        # Success message
        console.print("\n[bold green]🎉 Installation Complete![/bold green]")

        next_steps = [
            "[cyan]mcp-code-intelligence search 'your query'[/cyan] - Search your code",
            "[cyan]mcp-code-intelligence status[/cyan] - View project status",
        ]

        if not with_mcp:
            next_steps.append(
                "[cyan]mcp-code-intelligence install mcp[/cyan] - Add MCP integration"
            )
...
def _install_to_platform(
    ...
    try:
        ...
        if use_uv:
            ...
            command = "uv"
            args = ["run", "--directory", str(project_root), "mcp-code-intelligence", "mcp"]
        elif mcp_cmd:
...
def install_mcp(
    ...
    Auto-detects available platforms and installs mcp-code-intelligence as an MCP server.
    ...
    [bold cyan]Examples:[/bold cyan]
      ...
      [green]Auto-detect and install (recommended):[/green]
        $ mcp-code-intelligence install mcp
        $ mcp-code-intelligence install mcp --auto
...
def mcp_status(ctx: typer.Context) -> None:
    ...
    Displays which platforms have mcp-code-intelligence configured,
    ...
    try:
        ...
        for platform_info in detected:
            try:
                # Check if mcp-code-intelligence is configured
                installer = MCPInstaller(platform=platform_info.platform)
                server = installer.get_server("mcp-code-intelligence")
        ...
        console.print(
            "  mcp-code-intelligence install mcp                    # Install to auto-detected platform"
        )
        console.print(
            "  mcp-code-intelligence install mcp --all              # Install to all platforms"
        )
        console.print(
            "  mcp-code-intelligence install mcp --platform <name>  # Install to specific platform"
        )
...
def list_platforms(ctx: typer.Context) -> None:
    ...
    try:
        ...
        console.print(
            "  mcp-code-intelligence install mcp                    # Auto-detect"
        )
        console.print(
            "  mcp-code-intelligence install mcp --platform <name>  # Specific platform"
        )
        console.print(
            "  mcp-code-intelligence install mcp --all              # All platforms"
        )
