"""Watch command for MCP Code Intelligence CLI."""
...
def watch_main(
    ...
    Examples:
        ...
        mcp-code-intelligence watch /path/to/project --verbose
        mcp-code-intelligence watch --config custom-config.json
    """
    if verbose:
        logger.remove()
    ...
    try:
        ...
        if not project_manager.is_initialized():
            print_error(
                ...
                "Run 'mcp-code-intelligence init' first."
            )
            raise typer.Exit(1)
        ...
        print_error(
            f"No MCP Code Intelligence project found at {project_root}. "
            "Run 'mcp-code-intelligence init' to initialize."
        )
        raise typer.Exit(1)
...
def watch_status(
    ...
    try:
        ...
        if config.watch_files:
            ...
            print_info("Enable with: mcp-code-intelligence config set watch_files true")

    except ProjectNotFoundError:
        print_error(f"No MCP Code Intelligence project found at {project_root}")
        raise typer.Exit(1)
    except Exception as e:
...
def watch_enable(
    ...
    After enabling, you can use 'mcp-code-intelligence watch' to start monitoring.
    """
    try:
        project_manager = ProjectManager(project_root)
        if not project_manager.is_initialized():
        ...
        print_info("Start watching with: mcp-code-intelligence watch")
