...
def main(
    ...
    [green]Detailed status with all information:[/green]
        $ mcp-code-intelligence status --verbose

    [green]Check MCP integration:[/green]
        $ mcp-code-intelligence status --mcp

    [bold cyan]Diagnostics:[/bold cyan]

    [green]Full health check:[/green]
        $ mcp-code-intelligence status --health-check

    [green]Export metrics to JSON:[/green]
        $ mcp-code-intelligence status --metrics --json > metrics.json

    [green]Combined diagnostics:[/green]
        $ mcp-code-intelligence status --verbose --health-check --mcp

    [dim]💡 Tip: Use --metrics to see code quality analysis from 'mcp-code-intelligence analyze'[/dim]
    """
    ...
    async def run_status_with_timeout():
        ...
        try:
            ...
            # Re-raise to let handle_cli_errors catch it or allow typer.Exit
            raise typer.Exit(1)
...
"""
    ...
    if "mcp" in status_data:
        ...
        server_name = mcp_data.get("server_name", "mcp-code-intelligence")
...
async def check_mcp_integration(
    project_root: Path, server_name: str = "mcp-code-intelligence"
) -> dict[str, Any]:
    ...
    try:
        ...
        from mcp_code_intelligence.cli.commands.mcp import check_claude_code_available, get_claude_command
...
async def show_metrics_summary(
    ...
    try:
        ...
        storage_dir = project_root / ".mcp-code-intelligence"
                ...
                print_json(
                    {
                        ...
                        "message": "Run 'mcp-code-intelligence analyze' first",
                    }
                )
            else:
                console.print(
                    "[yellow]No metrics found. Run 'mcp-code-intelligence analyze' first.[/yellow]"
                )
            raise typer.Exit(1)
        ...
        if not snapshots:
            if json_output:
                ...
                console.print(
                    ...
                    "Run 'mcp-code-intelligence analyze' first.[/yellow]"
                )
            raise typer.Exit(1)
...
def check_dependencies() -> bool:
    ...
    if not all_available:
        ...
        console.print("  pip install mcp-code-intelligence")
        console.print()
        console.print("  [cyan]# Using uv (faster):[/cyan]")
        console.print("  uv pip install mcp-code-intelligence")
        console.print()
        ...
        if "tree_sitter_language_pack" in missing_deps:
            ...
            console.print("  brew reinstall mcp-code-intelligence")
            console.print()
