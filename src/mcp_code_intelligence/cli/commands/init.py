"""Init command for MCP Code Intelligence CLI."""
...
from mcp_code_intelligence.config.constants import (
...
def main(
    ...
    ✅ **Installs** mcp-code-intelligence in the current project
    ...
    [green]Custom file extensions:[/green]
        $ mcp-code-intelligence init --extensions .py,.js,.ts,.txt,.md
    ...
    [dim]💡 Tip: The command creates .mcp-code-intelligence/ for project config
    ...
    try:
        ...
        if project_manager.is_initialized() and not force:
            ...
            print_info(
                "Use --force to re-initialize or run 'mcp-code-intelligence status' to see current configuration"
            )
        ...
        console.print("\n[bold blue]🚀 MCP Code Intelligence Setup:[/bold blue]")
        ...
        if auto_index:
            ...
            try:
                ...
                print_info(
                    "You can run 'mcp-code-intelligence index' later to index your codebase"
                )
        else:
            print_info(
                "💡 Run 'mcp-code-intelligence index' to index your codebase when ready"
            )
        ...
        if mcp:
            ...
            try:
                ...
                from mcp_code_intelligence.cli.commands.mcp import create_project_claude_config
                ...
                if auto_indexing:
                    try:
                        ...
                        from mcp_code_intelligence.cli.commands.auto_index import _setup_auto_indexing
                        ...
                        print_info(
                            "You can set it up later with: mcp-code-intelligence auto-index setup"
                        )

            except Exception as e:
                print_warning(f"MCP integration failed: {e}")
                print_info(
                    "You can install it later with: mcp-code-intelligence mcp install"
                )
        ...
        if auto_index and mcp:
            ...
            next_steps = [
                ...
                "[cyan]mcp-code-intelligence status[/cyan] - Check indexing statistics",
            ]
            ...
            if not auto_index:
                steps.append(
                    "[cyan]mcp-code-intelligence index[/cyan] - Index your codebase"
                )
            steps.append(
                "[cyan]mcp-code-intelligence search 'your query'[/cyan] - Try semantic search"
            )
            steps.append("[cyan]mcp-code-intelligence status[/cyan] - Check project status")
            if not mcp:
                steps.append(
                    "[cyan]mcp-code-intelligence mcp install[/cyan] - Add Claude Code integration"
                )
...
def check_initialization(ctx: typer.Context) -> None:
    """Check if the current project is initialized for MCP Code Intelligence."""
    try:
        ...
        if project_manager.is_initialized():
            ...
            print_info("Run 'mcp-code-intelligence init' to initialize the project")
            raise typer.Exit(1)
...
async def run_init_setup(
    ...
    """
    from mcp_code_intelligence.config.defaults import DEFAULT_FILE_EXTENSIONS
    from mcp_code_intelligence.core.project import ProjectManager
    from mcp_code_intelligence.cli.output import print_project_info
    ...
    if auto_index:
        ...
        print_info("💡 Run 'mcp-code-intelligence index' to index your codebase when ready")
    ...
    if mcp:
        ...
        try:
            ...
            from mcp_code_intelligence.cli.commands.mcp import (
            ...
            if not check_claude_code_available():
                ...
                remove_cmd = [claude_cmd, "mcp", "remove", "mcp-code-intelligence"]
            ...
            print_warning(f"MCP integration failed: {e}")
            print_info("You can install it later with: mcp-code-intelligence mcp install")
...
def init_mcp_integration(
    ...
    server_name: str = typer.Option(
        "mcp-code-intelligence", "--name", help="Name for the MCP server"
    ),
    force: bool = typer.Option(
...
def _test_mcp_server(project_root: Path) -> None:
    ...
    try:
        ...
        from mcp_code_intelligence.cli.commands.mcp import get_mcp_server_command
