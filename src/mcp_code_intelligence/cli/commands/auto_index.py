...
from mcp_code_intelligence.core.factory import (
    ComponentFactory,
    ConfigurationService,
    DatabaseContext,
    handle_cli_errors,
)
from mcp_code_intelligence.core.git_hooks import GitHookManager
...
async def _show_auto_index_status(project_root: Path) -> None:
    ...
    if staleness_info["stale_files"] > 0:
        ...
        print_info(
            "Run 'mcp-code-intelligence auto-index check --auto-reindex' to update them"
        )
    else:
...
async def _setup_auto_indexing(
    ...
    if success_count > 0:
        ...
        print_info("\nNext steps:")
        print_info("• Use 'mcp-code-intelligence auto-index status' to check status")
        print_info("• Use 'mcp-code-intelligence auto-index check' to test manually")
