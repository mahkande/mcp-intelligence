...
Example:
    ...
    >>> from mcp_code_intelligence.analysis import ProjectMetrics
    >>> from mcp_code_intelligence.analysis.interpretation import (
...
from mcp_code_intelligence.analysis.visualizer.schemas import AnalysisExport, SmellLocation
...
class AnalysisInterpreter:
    ...
    def interpret(
        ...
        if focus == "summary":
            ...
            return self._interpret_hotspots(export, verbosity)
        elif focus == "cycles":
            return self._interpret_circular_dependencies(export, verbosity)
        else:
            return self._interpret_summary(export, verbosity)
    ...
    def _interpret_summary(self, export: LLMContextExport, verbosity: str) -> str:
        ...
        # Hotspots overview
        risk_hotspots = getattr(export.analysis.summary, "risk_hotspots", [])
        if risk_hotspots:
            lines.append("")
            lines.append("**Top Risk Hotspots**:")
            for path in risk_hotspots[:3]:
                lines.append(f"  - {Path(path).name} (Critical attention requested)")
        ...
        lines.append("> **AI Architect Directive:** As an AI architect, analyze the Health Score above. If it's below 80, prioritize refactoring the hotspots before suggesting new features.")
    ...
    def _interpret_hotspots(self, export: LLMContextExport, verbosity: str) -> str:
        """Generate hotspot-specific interpretation."""
        lines = ["# 🚨 High Risk Hotspots", ""]
        
        # We need the actual file metrics for risk scores
        # Since LLMContextExport captures AnalysisExport, we check files there
        hotspots = []
        for f in export.analysis.files:
            risk_score = getattr(f, "risk_score", 0.0)
            if risk_score > 0:
                hotspots.append(f)
        
        # Sort by risk score
        hotspots.sort(key=lambda x: getattr(x, "risk_score", 0.0), reverse=True)
        
        if not hotspots:
            return "No risk hotspots identified."

        lines.append("| Heatmap | File Path | Risk Score | Complexity | Churn (30d) | Status |")
        lines.append("|---------|-----------|------------|------------|------------|--------|")
        
        for f in hotspots[:5]:
            risk = getattr(f, "risk_score", 0.0)
            comp = getattr(f, "avg_complexity", 0.0)
            churn = getattr(f, "churn_count", 0)
            
            heatmap = "🔥🔥🔥" if risk > 100 else "🔥🔥" if risk > 70 else "🔥" if risk > 40 else "🟢"
            status = "🚨 CRITICAL" if risk > 100 else "⚠️ WARNING" if risk > 50 else "🟢 STABLE"
            lines.append(f"| {heatmap} | {Path(f.path).name} | {risk:.1f} | {comp:.1f} | {churn} | {status} |")
            
        lines.append("")
        lines.append("**AI Action Plan**:")
        lines.append("The system has identified the files above as high-risk 'Hotspots'. Please prepare refactoring suggestions to reduce complexity in the top 3 files with the highest risk scores.")
        
        if any(getattr(f, "risk_score", 0.0) > 100 for f in hotspots):
            lines.append("\n> [!CAUTION]")
            lines.append("> This file carries a critical risk due to a combination of high churn and excessive complexity. The next bug will likely occur here. Urgent refactoring is recommended.")

        return "\n".join(lines)

    def _interpret_circular_dependencies(self, export: LLMContextExport, verbosity: str) -> str:
        """Generate interpretation for circular dependencies with ASCII maps."""
        cycles = export.analysis.dependencies.circular_dependencies
        if not cycles:
        ...
        for i, cycle_data in enumerate(cycles, 1):
            chain = cycle_data.cycle # This is now list of [file, line] or similar
            lines.append(f"## Cycle {i}")
            
            # Build ASCII Box Map
            # Format:
            # ┌─> src/auth.py [Line 12: import storage]
            # │   └── src/storage.py [Line 45: from auth import User]
            # └──────┘
            ...
            for j, step in enumerate(chain[:-1]):
                if isinstance(step, (list, tuple)):
                    ...
                    path, line = step, "unknown"
                
                next_step = chain[j+1]
                if isinstance(next_step, (list, tuple)):
                    next_path = next_step[0]
                else:
                    next_path = next_step

                prefix = "┌─> " if j == 0 else "│   └── "
                ascii_map.append(f"{prefix}{path} [Line {line}: imports {Path(next_path).name}]")
        ...
        lines.append("Circular dependencies affect modularity and make it difficult to write unit tests. ")
        lines.append("Please examine the 'Box' map above and consider breaking the dependency through an interface or a common 'Types' file.")
