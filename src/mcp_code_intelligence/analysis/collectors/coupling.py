...
class ImportGraph:
    ...
    adjacency_list: dict[str, list[ImportEdge]] = field(default_factory=dict)

    def add_edge(self, from_file: str, to_module: str, line: int = 1) -> None:
        """Add directed edge from from_file to to_module with line info."""
        if from_file not in self.adjacency_list:
            self.adjacency_list[from_file] = []
        
        # Avoid duplicate edges
        if not any(e.module == to_module for e in self.adjacency_list[from_file]):
            self.adjacency_list[from_file].append(ImportEdge(to_module, line))
    ...
    def get_neighbors(self, file_path: str) -> list[ImportEdge]:
...
class CircularDependency:
    ...
    Attributes:
        cycle_chain: list[tuple[str, int]]  # List of (file_path, line_where_it_imports_next)
        cycle_length: Number of unique files in cycle

    Example:
        cycle = CircularDependency(
            cycle_chain=[("a.py", 10), ("b.py", 5), ("c.py", 20), ("a.py", 0)]
        )
    ...
    def format_chain(self) -> str:
        ...
        parts = []
        for path, line in self.cycle_chain[:-1]:
            parts.append(f"{path} (Line {line})")
        parts.append(self.cycle_chain[-1][0])
        return " → ".join(parts)
...
class CircularDependencyDetector:
    ...
    def _dfs(self, node: str) -> None:
        ...
        for edge in self.graph.get_neighbors(node):
            neighbor = edge.module
            if self._colors[neighbor] == NodeColor.GRAY:
                # Found cycle! Record path with line info
                self._record_cycle(neighbor, edge.line)
            elif self._colors.get(neighbor) == NodeColor.WHITE:
    ...
    def _record_cycle(self, cycle_start: str, last_line: int) -> None:
        ...
        # We need to find the line numbers for each step in the path
        # This is a bit complex since _path only stores names.
        # Let's reconstruct the (node, line) pairs for the cycle.
        cycle_chain = []
        path_nodes = self._path[start_index:]
        
        for i, node in enumerate(path_nodes):
            next_node = path_nodes[i+1] if i+1 < len(path_nodes) else cycle_start
            
            # Find the line in node that imports next_node
            line = 1
            for edge in self.graph.get_neighbors(node):
                if edge.module == next_node:
                    line = edge.line
                    break
            cycle_chain.append((node, line))
            
        cycle_chain.append((cycle_start, 0)) # End of loop
...
class EfferentCouplingCollector(MetricCollector):
    ...
    def __init__(self) -> None:
        ...
        self._imports: set[ImportEdge] = set()  # (module, line)
        self._internal_imports: set[ImportEdge] = set()
        self._external_imports: set[ImportEdge] = set()
    ...
    def _extract_import(self, node: Node, context: CollectorContext) -> None:
        ...
        if language == "python":
            ...
            if module_node:
                ...
                self._add_import(module_name, node.start_point[0] + 1, context)
            else:
                # Look for dotted_name child
                for child in node.children:
                    if child.type == "dotted_name":
                        module_name = child.text.decode("utf-8")
                        self._add_import(module_name, child.start_point[0] + 1, context)
                        ...
                        self._add_import(dots, child.start_point[0] + 1, context)
                        break
    ...
    def _add_import(self, module_name: str, line: int, context: CollectorContext) -> None:
        """Add import to tracking sets and classify as internal/external."""
        language = context.language
        edge = ImportEdge(module_name, line)

        # Add to all imports
        self._imports.add(edge)

        # Classify import
        if is_relative_import(module_name, language):
            # Relative import = internal
            self._internal_imports.add(edge)
        elif is_stdlib_module(module_name, language):
            # Standard library = external (but not third-party)
            self._external_imports.add(edge)
        else:
            self._external_imports.add(edge)

    def get_imported_modules(self) -> set[ImportEdge]:
        """Get set of all imported module names with lines."""
    ...
    def get_file_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "imports": sorted([e.module for e in self._imports]),
            "internal_imports": sorted([e.module for e in self._internal_imports]),
            "external_imports": sorted([e.module for e in self._external_imports]),
            "import_edges": [e._asdict() for e in self._imports]
        }
...
def build_import_graph(
    ...
    for file_path in files:
        ...
        imported_edges = efferent_collector.get_imported_modules()
        ...
        for edge in imported_edges:
            ...
            # Future: store line in reverse graph if needed
