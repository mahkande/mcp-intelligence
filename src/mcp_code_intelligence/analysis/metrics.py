...
class FileMetrics:
    ...
    Attributes:
        ...
        churn_count: Number of commits in the last 30 days
        author_count: Number of unique authors in the last 30 days
        risk_score: Computed risk score (churn * 1.5 + complexity * 2.0)
    """

    file_path: str
    language: str = "python"
    is_valid: bool = True  # Successfully analyzed
    total_lines: int = 0
    code_lines: int = 0
    comment_lines: int = 0
    blank_lines: int = 0
...
class ProjectMetrics:
    ...
    Attributes:
        ...
        risk_hotspots: List of file paths with highest risk score (top 10)
    """
    ...
    risk_hotspots: list[str] = field(default_factory=list)  # Top 10 risk files
    ...
    def compute_aggregates(self) -> None:
        ...
        # Identify risk hotspots (top 10 highest risk score)
        risk_files = self.get_risk_hotspots(limit=10)
        self.risk_hotspots = [f.file_path for f in risk_files]
    ...
    def get_risk_hotspots(self, limit: int = 10) -> list[FileMetrics]:
        """Return top N files with highest risk score.

        RiskScore = (ChangeFrequency * 1.5) + (AvgComplexity * 2.0)

        Args:
            limit: Maximum number of hotspots to return

        Returns:
            List of FileMetrics sorted by risk score (highest first)
        """
        sorted_files = sorted(
            self.files.values(), key=lambda f: f.risk_score, reverse=True
        )
        return sorted_files[:limit]
