...
class ExportMetadata(BaseModel):
    ...
    Attributes:
        ...
        tool_version: mcp-code-intelligence version that generated the export
    ...
    tool_version: str = Field(
        description="Version of mcp-code-intelligence that generated this export"
    )
...
class MetricsSummary(BaseModel):
    ...
    risk_hotspots: list[str] = Field(
        default_factory=list, description="Top 10 most risky files"
    )
...
class FileDetail(BaseModel):
    ...
    # Hotspot metrics
    risk_score: float = Field(default=0.0, description="Risk score (churn + complexity)")
    churn_count: int = Field(default=0, description="Commits in last 30 days")
    author_count: int = Field(default=0, description="Unique authors in last 30 days")
...
class CyclicDependency(BaseModel):
    ...
    cycle: list[Any] = Field(
        min_length=2, description="List of files in the cycle (ordered, may include line numbers)"
    )
