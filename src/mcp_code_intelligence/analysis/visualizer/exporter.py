...
Example:
    ...
    >>> from mcp_code_intelligence.analysis.visualizer import JSONExporter
    >>>
...
from mcp_code_intelligence.analysis.visualizer.schemas import (
    AnalysisExport,
    ClassMetrics,
    CyclicDependency,
    DependencyEdge,
...
if TYPE_CHECKING:
    from mcp_code_intelligence.analysis.metrics import ChunkMetrics, FileMetrics, ProjectMetrics
    from mcp_code_intelligence.analysis.storage.metrics_store import MetricsStore
    from mcp_code_intelligence.analysis.storage.trend_tracker import TrendTracker
...
class JSONExporter:
    ...
    def _create_summary(self, project_metrics: ProjectMetrics) -> MetricsSummary:
        ...
        return MetricsSummary(
            ...
            risk_hotspots=getattr(project_metrics, "risk_hotspots", []),
        )
    ...
    def _convert_file(self, file_metrics: FileMetrics) -> FileDetail:
        ...
        return FileDetail(
            ...
            risk_score=getattr(file_metrics, "risk_score", 0.0),
            churn_count=getattr(file_metrics, "churn_count", 0),
            author_count=getattr(file_metrics, "author_count", 0),
        )
    ...
    def _create_dependency_graph(
        ...
        # Circular dependencies - Use rich detector
        graph = build_import_graph_from_dict({f.file_path: f.coupling.imports for f in all_files})
        # IMPORTANT: to get line numbers we need to look into file_metrics.coupling.import_edges
        # Let's populate the graph properly
        import_graph_obj = ImportGraph()
        for f in all_files:
            import_graph_obj.add_node(f.file_path)
            # Use import_edges if available (populated in _convert_file)
            edges_data = getattr(f.coupling, "import_edges", [])
            for e in edges_data:
                # e is a dict from e._asdict()
                import_graph_obj.add_edge(f.file_path, e["module"], e["line"])
        
        detector = CircularDependencyDetector(import_graph_obj)
        detected_cycles = detector.detect_cycles()
        
        circular_dependencies: list[CyclicDependency] = []
        for cycle in detected_cycles:
            circular_dependencies.append(
                CyclicDependency(
                    cycle=cycle.cycle_chain,
