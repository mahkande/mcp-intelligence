...
class SchedulerManager:
    ...
    def install_scheduled_task(
        ...
        if task_name is None:
            ...
            task_name = f"mcp_code_intelligence_reindex_{safe_path}"
...
# MCP Code Intelligence Auto-Reindex - {task_name}
...
if command -v mcp-code-intelligence &> /dev/null; then
    ...
    "{python_path}" -m mcp_code_intelligence auto-index check --auto-reindex --max-files 10
else
    python3 -m mcp_code_intelligence auto-index check --auto-reindex --max-files 10
fi
"""

            # Write script to temp file
            script_dir = Path.home() / ".mcp-code-intelligence" / "scripts"
    ...
    def _install_windows_task(self, interval_minutes: int, task_name: str) -> bool:
        ...
        try:
            ...
            script_content = f"""# MCP Code Intelligence Auto-Reindex - {task_name}
Set-Location "{project_root}"

try {{
    if (Get-Command "mcp-code-intelligence" -ErrorAction SilentlyContinue) {{
        mcp-code-intelligence auto-index check --auto-reindex --max-files 10
    }} elseif (Test-Path "{python_path}") {{
        & "{python_path}" -m mcp_code_intelligence auto-index check --auto-reindex --max-files 10
    }} else {{
        python -m mcp_code_intelligence auto-index check --auto-reindex --max-files 10
    }}
}} catch {{
