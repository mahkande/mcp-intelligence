...
class GitManager:
    ...
    def get_current_branch(self) -> str | None:
        ...
        try:
            ...
            return None
    def get_churn_metrics(self, days: int = 30) -> dict[str, dict[str, int]]:
        """Get churn metrics (commit frequency and unique authors) for all files.
        
        Analyzes git history for the last N days.
        
        Args:
            days: Number of days to look back
            
        Returns:
            Dictionary mapping relative file paths to metrics:
            {
                "path/to/file.py": {
                    "commit_count": 12,
                    "author_count": 3
        ...
        try:
            ...
            for line in result.stdout.splitlines():
                ...
                if line.startswith("AUTH:"):
                    current_author = line[5:]
                    continue
                
                # It's a file path
                rel_path = line
                if rel_path not in metrics:
                    metrics[rel_path] = {
                        "commit_count": 0,
                        "authors": set()
                    }
                
                metrics[rel_path]["commit_count"] += 1
                if current_author:
                    metrics[rel_path]["authors"].add(current_author)
