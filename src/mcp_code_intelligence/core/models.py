...
class CodeChunk:
    ...
    def __post_init__(self) -> None:
        ...
        if self.content_hash is None:
            from mcp_code_intelligence.utils.hashing import calculate_content_md5
            self.content_hash = calculate_content_md5(self.content)
...
class SearchResult(BaseModel):
    ...
    base_score: float | None = Field(
        default=None, description="Original similarity score before boosts"
    )
    boost_score: float | None = Field(
        default=None, description="Total boost/penalty applied"
    )
...
class Directory:
    ...
    def id(self) -> str:
        ...
        return calculate_id_hash(str(self.path))
