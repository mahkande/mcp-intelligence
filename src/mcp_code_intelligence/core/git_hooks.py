...
class GitHookManager:
    ...
    def _uninstall_hook(self, hook_type: str) -> bool:
        ...
        try:
            ...
            if "# MCP Code Intelligence Hook" in content:
                if (
                    content.strip().startswith("#!/bin/bash")
                    and "# MCP Code Intelligence Hook" in content
                ):
    ...
    def _generate_hook_script(self, hook_type: str) -> str:
        ...
        script = f"""#!/bin/bash
# MCP Code Intelligence Hook - {hook_type}
...
if ! command -v mcp-code-intelligence &> /dev/null; then
    ...
    if $PYTHON_CMD -m mcp_code_intelligence --help &> /dev/null; then
        MCP_CMD="$PYTHON_CMD -m mcp_code_intelligence"
    else
        # Silently exit if not available
        exit 0
    fi
else
    MCP_CMD="mcp-code-intelligence"
fi
...
"""
    ...
    def _integrate_with_existing_hook(self, hook_file: Path, our_script: str) -> bool:
        ...
        try:
            ...
            if "# MCP Code Intelligence Hook" in existing_content:
    ...
    def _remove_from_existing_hook(self, hook_file: Path) -> bool:
        ...
        try:
            ...
            for line in lines:
                if "# MCP Code Intelligence Hook" in line:
                    skip_section = True
                    continue
        ...
        for hook_type in hook_types:
            ...
            if hook_file.exists():
                try:
                    ...
                    content = hook_file.read_text()
                    hook_status["has_mcp_hook"] = "# MCP Code Intelligence Hook" in content
                    hook_status["is_mcp_only"] = (
                        hook_status["has_mcp_hook"]
                        and content.strip().startswith("#!/bin/bash")
                        and content.count("# MCP Code Intelligence Hook") == 1
                    )
                except Exception:
                    pass
