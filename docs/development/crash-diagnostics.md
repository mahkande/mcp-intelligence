...
MCP Code Intelligence includes signal handlers to catch segmentation faults (SIGSEGV) and provide helpful error messages to users. This is especially important because the CLI uses native libraries like ChromaDB, sentence-transformers, and tree-sitter that can occasionally crash due to corrupted data or memory issues.
...
The signal handler is registered early in the application startup in `src/mcp_code_intelligence/cli/main.py`:
...
**Solution:**
```bash
mcp-code-intelligence index clean
...
**Solution:**
```bash
pip install -U mcp-code-intelligence
```
...
def test_segfault_handler_message():
    ...
    from mcp_code_intelligence.cli.main import _handle_segfault
    ...
    assert "mcp-code-intelligence index clean" in error_output
```
