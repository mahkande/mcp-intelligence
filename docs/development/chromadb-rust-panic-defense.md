...
This document describes the two-layer defense system implemented to prevent `mcp-code-intelligence index` crashes due to ChromaDB Rust panics caused by database corruption.

## Problem

The `mcp-code-intelligence index` command would crash with Rust panic errors like:
...
**Location**: `src/mcp_code_intelligence/core/database.py::_detect_and_recover_corruption()`
...
**Location**: `src/mcp_code_intelligence/core/database.py::initialize()`
        ...
        if any(pattern in error_msg for pattern in rust_panic_patterns):
            ...
            try:
                ...
                raise DatabaseError(
                    ...
                    f"Please run 'mcp-code-intelligence reset' manually. "
                    f"Error: {retry_error}"
                ) from retry_error
...
**Location**: `src/mcp_code_intelligence/core/database.py::initialize()` (outer except block)
...
4. **Clear error messages**: If recovery fails, suggest manual `mcp-code-intelligence reset`

## Recovery Process

When corruption is detected, `_recover_from_corruption()`:

1. Creates timestamped backup: `.mcp-code-intelligence_backup/backup_{timestamp}/`
...
INFO: ✓ Created backup at .mcp-code-intelligence_backup/backup_1234567890
INFO: ✓ Cleared corrupted index
INFO: ✓ Index directory recreated
DEBUG: ChromaDB initialized at .mcp-code-intelligence/
```
...
Please run 'mcp-code-intelligence reset' manually. Error: ...
```
...
- `src/mcp_code_intelligence/core/database.py`
