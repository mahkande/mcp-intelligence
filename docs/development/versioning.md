...
MCP Code Intelligence follows [Semantic Versioning 2.0.0](https://semver.org/) (SemVer) for version numbering. This ensures predictable and meaningful version numbers for users and developers.
...
mcp-code-intelligence search "query"     # v1.x
mcp-code-intelligence query "search"     # v2.x (breaking)
```
...
mcp-code-intelligence index --language go
```
...
uv run python -c "from mcp_code_intelligence import __version__; print(__version__)"
...
git add src/mcp_code_intelligence/__init__.py CHANGELOG.md
...
vim src/mcp_code_intelligence/__init__.py
vim CHANGELOG.md
...
### CLI Changes
**Old**: `mcp-code-intelligence search "query"`
**New**: `mcp-code-intelligence query "search"`
...
print(f"MCP Code Intelligence v{__version__}")
...
def get_version():
    ...
    try:
        return pkg_resources.get_distribution("mcp-code-intelligence").version
