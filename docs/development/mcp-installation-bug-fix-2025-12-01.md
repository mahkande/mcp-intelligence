...
**Issue**: Server was hardcoded as "mcp" instead of "mcp-code-intelligence"
...
**Location**: `register_with_claude_cli()` in both `install.py` and `setup.py`
**Issue**: Command was hardcoded as `uv run python -m mcp_code_intelligence.mcp.server` instead of `mcp-code-intelligence mcp`
**Impact**: Only worked for uv development environments, failed for pipx and homebrew installations

**Fixed Lines**:
- `install.py` lines 151-153: Changed from `["uv", "run", "python", "-m", "mcp_code_intelligence.mcp.server", ...]` to `["mcp-code-intelligence", "mcp", ...]`
- `setup.py` lines 161-163: Same change as above

### Bug #3: Missing Command Availability Check
**Location**: `register_with_claude_cli()` in both `install.py` and `setup.py`
**Issue**: Checked for `uv` availability instead of `mcp-code-intelligence` command
**Impact**: Would fail silently if mcp-code-intelligence wasn't in PATH

**Fixed Lines**:
- `install.py` line 121: `if not shutil.which("mcp-code-intelligence"):`
- `setup.py` line 124: `if not shutil.which("mcp-code-intelligence"):`

## Changes Made

### 1. `/src/mcp_code_intelligence/cli/commands/install.py`

#### Added `server_name` parameter to `register_with_claude_cli()`:
```python
def register_with_claude_cli(
    project_root: Path,
    server_name: str = "mcp-code-intelligence",  # NEW PARAMETER
...
# AFTER: Check for mcp-code-intelligence command
if not shutil.which("mcp-code-intelligence"):
    logger.warning("mcp-code-intelligence command not in PATH, falling back to manual JSON configuration")
    return False
```
...
cmd = [
    ...
    "--", "uv", "run", "python", "-m", "mcp_code_intelligence.mcp.server",
...
cmd = [
    ...
    "--env", f"MCP_ENABLE_FILE_WATCHING={'true' if enable_watch else 'false'}",
    "--", "mcp-code-intelligence", "mcp", str(project_root.absolute()),
]
```
...
### 2. `/src/mcp_code_intelligence/cli/commands/setup.py`

Applied identical changes as in `install.py`:
- Added `server_name` parameter to function signature
- Changed command availability check from `check_uv_available()` to `shutil.which("mcp-code-intelligence")`
- Changed server name in remove command from hardcoded "mcp" to `server_name` variable
- Changed command construction from `uv run python -m ...` to `mcp-code-intelligence mcp`
- Updated function call to pass `server_name="mcp-code-intelligence"`
...
```
**Command registered**: `mcp-code-intelligence mcp /path/to/project`

### ✅ 2. Homebrew
```bash
brew install mcp-code-intelligence  # When available
mcp-code-intelligence install claude-code
```
**Command registered**: `mcp-code-intelligence mcp /path/to/project`
...
### Maintained:
- JSON config fallback when `mcp-code-intelligence` command not in PATH
...
- ✅ Server registered as "mcp-code-intelligence" (not "mcp")
- ✅ Command uses "mcp-code-intelligence mcp {project_root}" format
...
- Affected files:
  - `/src/mcp_code_intelligence/cli/commands/install.py` (lines 103-175)
  - `/src/mcp_code_intelligence/cli/commands/setup.py` (lines 104-175)
...
- ✅ Consistent server naming "mcp-code-intelligence"
