...
1. **Revert Backend**: Remove FastAPI, restore SimpleHTTPRequestHandler
   ```bash
   git checkout HEAD~1 -- src/mcp_code_intelligence/cli/commands/visualize/server.py
   ```

2. **Revert Frontend**: Restore direct `chunk-graph.json` fetch
   ```bash
   git checkout HEAD~1 -- src/mcp_code_intelligence/cli/commands/visualize/templates/scripts.py
   ```
...
4. **Regenerate HTML**: Re-export visualization
   ```bash
   uv run mcp-code-intelligence visualize export
   ```
...
uv run python3 -c "
from src.mcp_code_intelligence.cli.commands.visualize.templates.base import generate_html_template
from pathlib import Path
html = generate_html_template()
Path('.mcp-code-intelligence/visualization/index.html').write_text(html)
"

# Test server
uv run mcp-code-intelligence visualize serve --port 8088
```
