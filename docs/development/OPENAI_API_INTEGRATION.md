...
### 1. **Core LLM Client** (`src/mcp_code_intelligence/core/llm_client.py`)
...
### 2. **Configuration Storage** (`src/mcp_code_intelligence/core/config_utils.py`)
...
2. Config file (`~/.mcp-code-intelligence/config.json`)

### 3. **Settings Schema** (`src/mcp_code_intelligence/config/settings.py`)
...
### 4. **Chat Command** (`src/mcp_code_intelligence/cli/commands/chat.py`)
...
mcp-code-intelligence chat "where is similarity_threshold set?"

# Force OpenAI
mcp-code-intelligence chat "question" --provider openai

# Force OpenRouter
mcp-code-intelligence chat "question" --provider openrouter

# Custom model
mcp-code-intelligence chat "question" --model gpt-4o
mcp-code-intelligence chat "question" --model anthropic/claude-3.5-sonnet
```
...
### 5. **Setup Command** (`src/mcp_code_intelligence/cli/commands/setup.py`)
...
API keys are stored in `~/.mcp-code-intelligence/config.json`:
...
mcp-code-intelligence chat "question"
```
...
mcp-code-intelligence chat "question"  # Auto-detects OpenAI

# Option B: Use both (OpenAI preferred by default)
export OPENAI_API_KEY="sk-..."
export OPENROUTER_API_KEY="sk-or-..."
mcp-code-intelligence chat "question"  # Uses OpenAI
...
- [ ] **OpenAI API Key Setup**
  ...
  - [ ] Config file: `mcp-code-intelligence setup` → Option 1
  - [ ] Verify chat works: `mcp-code-intelligence chat "test"`

- [ ] **OpenRouter API Key Setup**
  - [ ] Environment variable: `export OPENROUTER_API_KEY="sk-or-..."`
  - [ ] Config file: `mcp-code-intelligence setup` → Option 2
  - [ ] Verify chat works: `mcp-code-intelligence chat "test"`
...
mcp-code-intelligence chat "where is the similarity_threshold parameter set?"

# Test OpenRouter
export OPENROUTER_API_KEY="your-key"
mcp-code-intelligence chat "where is the similarity_threshold parameter set?" --provider openrouter

# Test auto-detection (both keys set)
export OPENAI_API_KEY="your-key"
export OPENROUTER_API_KEY="your-key"
mcp-code-intelligence chat "test"  # Should use OpenAI

# Test provider override
mcp-code-intelligence chat "test" --provider openrouter  # Should use OpenRouter
```

## Files Modified

1. **`src/mcp_code_intelligence/core/llm_client.py`** - Multi-provider LLM client
2. **`src/mcp_code_intelligence/core/config_utils.py`** - OpenAI key storage functions
3. **`src/mcp_code_intelligence/config/settings.py`** - New config fields
4. **`src/mcp_code_intelligence/cli/commands/setup.py`** - Interactive setup for both providers
5. **`src/mcp_code_intelligence/cli/commands/chat.py`** - Provider selection support
