...
The `--files '*.ts'` filter in `mcp-code-intelligence search` command treated the pattern as a literal string instead of a glob pattern.

**Buggy behavior:**
```bash
mcp-code-intelligence search --files '*.ts' "query"
...
In `src/mcp_code_intelligence/cli/commands/search.py` (line 337-339):
...
**File:** `src/mcp_code_intelligence/cli/commands/search.py`
...
mcp-code-intelligence search --files 'src/*.py' "database"

# Match specific file by name
mcp-code-intelligence search --files 'search.py' "function"

# Combine with other filters
mcp-code-intelligence search --files '*.py' --language python "async function"
```

## Files Changed

1. `src/mcp_code_intelligence/cli/commands/search.py` - Main fix implementation
...
- `src/mcp_code_intelligence/core/database.py` - `_build_where_clause()` still treats file_path as exact match (correct behavior)
- `src/mcp_code_intelligence/core/search.py` - Search engine unchanged
