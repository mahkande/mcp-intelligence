# CLAUDE.md - MCP Code Intelligence Project Guide

**Primary Instructions for Claude Code & Claude MPM Integration**

> 🎯 **Priority System**: 🔴 Critical | 🟡 High | 🟢 Medium | ⚪ Optional

This file provides comprehensive guidance for Claude Code (claude.ai/code) and Claude MPM when working with the MCP Code Intelligence codebase.

## 🔴 Project Overview (CRITICAL)

**MCP Code Intelligence** is a CLI-first semantic code search tool with MCP (Model Context Protocol) integration. It provides intelligent code search using vector embeddings and AST-aware parsing for **8 languages**: Python, JavaScript, TypeScript, Dart/Flutter, PHP, Ruby, HTML, and Markdown/Text.
...
mcp-code-intelligence init --no-mcp

# Skip automatic indexing
mcp-code-intelligence init --no-auto-index

# Custom file extensions
mcp-code-intelligence init --extensions .py,.js,.ts,.dart

# Combine options
mcp-code-intelligence init --no-auto-index --no-mcp
```

**Deprecated commands:** The `install` command has been replaced by `init`. Use `mcp-code-intelligence init` instead.
...
```bash
# Core Workflow
mcp-code-intelligence init          # Initialize project
mcp-code-intelligence doctor        # Check system health
mcp-code-intelligence status        # Show project status
mcp-code-intelligence search "query" # Search code
mcp-code-intelligence index         # Index codebase
mcp-code-intelligence mcp           # MCP integration
mcp-code-intelligence config        # Configuration
mcp-code-intelligence help          # Get help
mcp-code-intelligence version       # Show version
```

### 🔴 Search Commands (PRIMARY USER INTERFACE)
```bash
# Basic Search
mcp-code-intelligence search "authentication"           # Semantic search
mcp-code-intelligence search "auth" --language python   # Filter by language
mcp-code-intelligence search "error" --files "src/*.py" # Filter by files

# Advanced Search
mcp-code-intelligence search "file.py" --similar        # Find similar code
mcp-code-intelligence search "impl rate limit" --context # Context search

# Search Subcommands
mcp-code-intelligence search interactive   # Interactive mode
mcp-code-intelligence search history       # View search history
mcp-code-intelligence search favorites list # Manage favorites
```

### 🔴 Index Commands (CODE INDEXING)
```bash
# Indexing
mcp-code-intelligence index              # Index all files
mcp-code-intelligence index --incremental # Smart incremental index

# Index Subcommands
mcp-code-intelligence index watch        # Watch for changes
mcp-code-intelligence index auto setup   # Setup auto-indexing
mcp-code-intelligence index health       # Check index health
```

### 🔴 Configuration Commands
```bash
# Config Management
mcp-code-intelligence config show        # Show current config
mcp-code-intelligence config set key val # Set config value
mcp-code-intelligence config models      # List embedding models
mcp-code-intelligence config reset       # Reset to defaults
```
...
#### 🔴 1. CLI Layer (`src/mcp_code_intelligence/cli/`) - USER INTERFACE
...
#### 🔴 2. Core Engine (`src/mcp_code_intelligence/core/`) - BUSINESS LOGIC
...
#### 🟡 3. Parser System (`src/mcp_code_intelligence/parsers/`) - LANGUAGE SUPPORT
...
#### 🔴 4. MCP Integration (`src/mcp_code_intelligence/mcp/`) - CLAUDE INTEGRATION
...
from mcp_code_intelligence.parsers.base import BaseParser
...
from mcp_code_intelligence.core.database import PooledChromaVectorDatabase
...
mcp-code-intelligence auto-index setup --method all
...
Project configuration is stored in `.mcp-code-intelligence/config.json`:
- Auto-created during `mcp-code-intelligence init`
...
```json
{
  "mcpServers": {
    "mcp-code-intelligence": {
      "command": "uv",
      "args": ["run", "mcp-code-intelligence", "mcp"],
      "cwd": "/Users/masa/Projects/managed/mcp-code-intelligence"
    }
  }
}
```
...
uv run mcp-code-intelligence mcp
...
1. **Create parser file**: `src/mcp_code_intelligence/parsers/new_language.py`
...
1. **Create command**: `src/mcp_code_intelligence/cli/commands/new_command.py`
...
- **Version**: `src/mcp_code_intelligence/__init__.py` - single source of truth
- **Entry Point**: `src/mcp_code_intelligence/cli/main.py` - CLI application root
...
- **Project Config**: `.mcp-code-intelligence/config.json` - per-project settings
- **Vector Database**: `.mcp-code-intelligence/chroma_db/` - indexed code storage
...
# Test MCP server locally
uv run mcp-code-intelligence mcp --debug

# Detailed project status
mcp-code-intelligence status --verbose --debug

# Force complete reindex
mcp-code-intelligence index --force --verbose

# Test auto-indexing (dry run)
mcp-code-intelligence auto-index check --dry-run --verbose
```
...
1. **CLI Layer** - Major refactoring and UX improvements
   - `src/mcp_code_intelligence/cli/main.py` (540 lines changed)
...
5. **Configuration** - Enhanced project settings
   - `src/mcp_code_intelligence/config/constants.py` (new constants)
