...
```
src/mcp_code_intelligence/cli/commands/visualize/templates/
...
For a file at `src/mcp_code_intelligence/cli/commands/visualize/scripts.py`:

```
🏠 Root / src / mcp_code_intelligence / cli / commands / visualize / scripts.py
^^^^^^^   ^^^   ^^^^^^^^^^^^^^^^^   ^^^   ^^^^^^^^   ^^^^^^^^^   ^^^^^^^^^^
...
2. **Open in browser**: `open .mcp-code-intelligence/visualization/index.html`
3. **Click a deeply nested file**: e.g., `src/mcp_code_intelligence/cli/commands/visualize/scripts.py`
4. **Verify breadcrumbs show**: Root / src / mcp_code_intelligence / cli / commands / visualize / scripts.py
