...
- Confusion with multiple projects (e.g., EDGAR vs mcp-code-intelligence)
...
1. **`src/mcp_code_intelligence/cli/commands/install.py`** (150+ lines)
...
2. **`src/mcp_code_intelligence/mcp/server.py`** (30 lines)
...
```
1. Check for .mcp-code-intelligence/ in current directory
   → Found? Use current directory ✓

2. Walk up to find .git/ directory
   → Found? Check if .mcp-code-intelligence/ exists at git root
   → Yes? Use git root ✓
...
### Design Decisions
✅ Prefer `.mcp-code-intelligence/` over git root (supports monorepos)
...
### Core Implementation
- `/src/mcp_code_intelligence/cli/commands/install.py`
- `/src/mcp_code_intelligence/mcp/server.py`
