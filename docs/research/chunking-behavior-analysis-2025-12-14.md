# MCP Code Intelligence: Chunking Behavior Analysis
...
The mcp-code-intelligence indexer **correctly extracts classes AND individual methods as separate chunks**. The SemanticIndexer class (1208 lines) is indexed as **33 total chunks**:
...
**File**: `src/mcp_code_intelligence/parsers/python.py`
...
**File**: `src/mcp_code_intelligence/parsers/python.py` (lines 137-182)
...
**Query Results** (actual data from `.mcp-code-intelligence/chroma.sqlite3`):
...
**File**: `src/mcp_code_intelligence/core/indexer.py` (lines 1124-1229)
...
The mcp-code-intelligence indexer correctly implements a **dual-level chunking strategy** where classes are indexed both as complete entities AND as collections of individual methods. This is intentional design that enables both broad class-level searches and granular method-level searches.
...
{
  "remember": [
    "mcp-code-intelligence uses dual-level chunking: classes are indexed both as complete entities AND as individual methods",
