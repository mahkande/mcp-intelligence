...
**Ticket**: [Issue #75](https://github.com/bobmatnyc/mcp-code-intelligence/issues/75)
...
1. **Update DEFAULT_FILE_EXTENSIONS** in `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/config/defaults.py` (lines 13-39)
2. **Generate extensions dynamically** from parser registry instead of hardcoded list
3. **Update ProjectConfig default** in `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/config/settings.py` (line 17)
...
**File**: `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/config/defaults.py`
**Lines**: 13-39
...
**File**: `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/config/settings.py`
**Lines**: 16-19
...
**File**: `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/cli/commands/init.py`
...
**Output Location**: `.mcp-code-intelligence/config.json`
...
**File**: `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/parsers/registry.py`
...
def get_default_config_path(project_root: Path) -> Path:
    ...
    return project_root / ".mcp-code-intelligence" / "config.json"
```

**Location**: `<project_root>/.mcp-code-intelligence/config.json`
...
3. Write JSON to `.mcp-code-intelligence/config.json`
...
```json
{
  ...
  "index_path": "/path/to/project/.mcp-code-intelligence",
  "file_extensions": [
    ".py",
    ".js",
    ".ts",
...
**File**: `src/mcp_code_intelligence/config/defaults.py`
...
**File**: `src/mcp_code_intelligence/config/settings.py`
**Lines**: 16-19
...
# Check config.json
cat .mcp-code-intelligence/config.json | jq '.file_extensions | length'
# Should be 30+ extensions (not just 5)

cat .mcp-code-intelligence/config.json | jq '.file_extensions' | grep -c "dart"
...
mcp-code-intelligence init --extensions .py,.js,.ts --force

# Check config.json
cat .mcp-code-intelligence/config.json | jq '.file_extensions'
...
**Before**:
- `mcp-code-intelligence init` (no flags) → indexes 27 extensions
- ProjectConfig Pydantic default → 5 extensions (INCONSISTENT!)

**After (Option A - Dynamic)**:
- `mcp-code-intelligence init` (no flags) → indexes 30+ extensions (all parsers + fallback)
- ProjectConfig Pydantic default → same 30+ extensions (CONSISTENT!)
- Auto-updates when new parsers added

**After (Option B - Static)**:
- `mcp-code-intelligence init` (no flags) → indexes 33 extensions (hardcoded list)
...
3. **Result**: `mcp-code-intelligence init` will index all 30+ supported extensions by default
