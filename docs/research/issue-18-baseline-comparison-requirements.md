...
**Issue**: [#18 - Baseline comparison](https://github.com/bobmatnyc/mcp-code-intelligence/issues/18)
...
Issue #18 implements **baseline comparison capability** for the `mcp-code-intelligence analyze` command, enabling developers and CI/CD pipelines to track metric changes over time by comparing current code quality metrics against a previous known-good state (baseline). This feature unblocks quality gate enforcement and technical debt tracking.
...
- **Command Interface**: `mcp-code-intelligence analyze --save-baseline <name>`
- **Behavior**:
...
- **Command Interface**: `mcp-code-intelligence analyze --compare-baseline <name>`
- **Behavior**:
...
- **Examples**:
  ...
  mcp-code-intelligence analyze --save-baseline "main-branch"

  # Compare current branch against main
  mcp-code-intelligence analyze --compare-baseline "main-branch"

  # Create release baseline
  mcp-code-intelligence analyze --save-baseline "v1.2.0"

  # List available baselines
  mcp-code-intelligence analyze --list-baselines
  ```
...
```json
{
  ...
  "project": {
    "path": "/Users/masa/Projects/mcp-code-intelligence",
    "language_count": 3,
    "file_count": 42
  },
  "aggregate_metrics": {
  ...
  "files": {
    "src/mcp_code_intelligence/core/indexer.py": {
      "path": "src/mcp_code_intelligence/core/indexer.py",
      "language": "python",
      "file_metrics": {
        "lines_of_code": 412,
        "functions": 8,
...
**Primary**: `~/.mcp-code-intelligence/baselines/`
- Directory structure:
  ```
...
**Project-specific** (Optional): `.mcp-code-intelligence/baselines/` in project root
...
```bash
mcp-code-intelligence analyze --save-baseline <name> [options]
```
...
5. Serialize to JSON
6. Save to `~/.mcp-code-intelligence/baselines/{name}.json`
...
✓ Baseline saved: ~/.mcp-code-intelligence/baselines/main-branch.json (156 KB)
  Files: 42
  Functions: 156
...
```bash
mcp-code-intelligence analyze --compare-baseline <name> [options]
```
...
1. main-branch (2025-12-11 15:30 UTC)
   ...
   Location: ~/.mcp-code-intelligence/baselines/main-branch.json

2. v1.2.0 (2025-12-09 10:15 UTC)
   Files: 40 | Functions: 150 | Max CC: 25
   Location: ~/.mcp-code-intelligence/baselines/v1.2.0.json

3. pre-refactor (2025-12-08 14:45 UTC)
   Files: 45 | Functions: 172 | Max CC: 32
   Location: ~/.mcp-code-intelligence/baselines/pre-refactor.json
```

### 3.4 Inspect Baseline Command

```bash
mcp-code-intelligence analyze --inspect-baseline <name>
```
...
mcp-code-intelligence analyze --changed-only --compare-baseline main-branch

# Save current analysis as baseline after successful test
mcp-code-intelligence analyze --save-baseline "post-review"

# Create baseline from git branch analysis
mcp-code-intelligence analyze --baseline main --save-baseline "main-current"
```
...
Warning: Baseline 'main-branch' uses different metric version
  Baseline version: 1.0 (created with mcp-code-intelligence v0.16.1)
  Current version: 1.1 (running mcp-code-intelligence v0.18.0)
...
Recommendation: Re-create baseline with current version:
  mcp-code-intelligence analyze --save-baseline main-branch --overwrite
```
...
**New files**:
- `src/mcp_code_intelligence/analysis/baseline/manager.py` - BaselineManager class
- `src/mcp_code_intelligence/analysis/baseline/comparator.py` - BaselineComparator class
- `src/mcp_code_intelligence/analysis/baseline/__init__.py`
- `tests/unit/analysis/baseline/test_manager.py`
- `tests/unit/analysis/baseline/test_comparator.py`

**Modified files**:
- `src/mcp_code_intelligence/cli/commands/analyze.py` - Add baseline flags
- `src/mcp_code_intelligence/analysis/reporters/console.py` - Add comparison output
...
### Design Document
- **Location**: `docs/research/mcp-code-intelligence-structural-analysis-design.md`
...
### GitHub Issues
- **Epic #12** (Quality Gates): `https://github.com/bobmatnyc/mcp-code-intelligence/issues/12`
- **Issue #18**: `https://github.com/bobmatnyc/mcp-code-intelligence/issues/18`

### Project Milestones
- **v0.18.0 - Quality Gates**: https://github.com/bobmatnyc/mcp-code-intelligence/milestones
