...
**Key Findings**:
1. ✅ MCP server entry point is `python -m mcp_code_intelligence.mcp.server`
...
**Primary Entry Point**: `mcp_code_intelligence.mcp.server`
- **Location**: `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/mcp/server.py`
- **Module Path**: `python -m mcp_code_intelligence.mcp.server`
...
```toml
[project.scripts]
mcp-code-intelligence = "mcp_code_intelligence.cli.main:cli_with_suggestions"
```
...
- `<name>` - Server identifier (e.g., "mcp-code-intelligence")
...
### Current mcp-code-intelligence Usage (from MCP_SETUP.md)

**Current Manual JSON Config** (Lines 18-26):
```bash
claude mcp add-json mcp-code-intelligence '{
  "command": "/Users/masa/Projects/managed/mcp-code-intelligence/.venv/bin/python",
  "args": ["-m", "mcp_code_intelligence.mcp.server", "/Users/masa/Projects/managed/mcp-code-intelligence"],
...
**Command**:
```bash
claude mcp add --transport stdio mcp-code-intelligence \
  --env MCP_ENABLE_FILE_WATCHING=true \
  -- uv run mcp-code-intelligence mcp
```

**Why This Works**:
- `uv run mcp-code-intelligence mcp` - Uses the CLI entry point defined in pyproject.toml
...
claude mcp add --transport stdio mcp-code-intelligence \
  --env MCP_ENABLE_FILE_WATCHING=true \
  -- uv run python -m mcp_code_intelligence.mcp.server {PROJECT_ROOT}
```

**Why This Works**:
- `uv run python` - Runs Python from the uv-managed environment
- `-m mcp_code_intelligence.mcp.server` - Executes the MCP server module
...
claude mcp add --transport stdio mcp-code-intelligence \
  --env MCP_ENABLE_FILE_WATCHING=true \
  -- python -m mcp_code_intelligence.mcp.server {PROJECT_ROOT}
```

**Why This Works**:
- Assumes `mcp-code-intelligence` is installed in PATH via pipx/uv tool
...
def generate_mcp_server_command(
    ...
    if platform.uv_available:
        cmd = f"uv run python -m mcp_code_intelligence.mcp.server"
    ...
    # Priority 2: pipx (if uv not available)
        ...
        cmd = f"{platform.python_executable} -m mcp_code_intelligence.mcp.server"

    # Priority 3: Direct python (fallback)
    else:
        cmd = f"python -m mcp_code_intelligence.mcp.server"
...
def register_mcp_server_native(
    ...
    # Example: "uv run python -m mcp_code_intelligence.mcp.server /path/to/project"
...
def automatic_setup(
    ...
    Examples:
        $ mcp-code-intelligence setup
        $ mcp-code-intelligence setup --skip-index
        $ mcp-code-intelligence setup --scope user
        $ mcp-code-intelligence setup --no-watch
    """
...
def show_manual_setup_instructions(
    ...
    console.print("  • Check mcp-code-intelligence is in PATH")
    console.print("  • Verify project is initialized: mcp-code-intelligence status")
```
...
```bash
claude mcp add --transport stdio mcp-code-intelligence \
  -- cmd /c uv run python -m mcp_code_intelligence.mcp.server {PROJECT_ROOT}
```
...
def test_generate_mcp_server_command_with_uv():
    ...
    assert "mcp_code_intelligence.mcp.server" in cmd
...
def test_setup_command_full_workflow(tmp_path):
    ...
    assert (tmp_path / ".mcp-code-intelligence" / "config.json").exists()
    ...
    if mcp_json.exists():
        ...
        assert "mcp-code-intelligence" in config["mcpServers"]
...
Registering: claude mcp add --transport stdio mcp-code-intelligence --env MCP_ENABLE_FILE_WATCHING=true -- uv run python -m mcp_code_intelligence.mcp.server /Users/user/my-project
✅ MCP server 'mcp-code-intelligence' registered successfully!
...
⚠️  Recommendations:
  ...
  • Alternatively, install via pipx: pipx install mcp-code-intelligence
...
```
IF uv_available:
    base_command = "uv run python -m mcp_code_intelligence.mcp.server"
...
ELSE IF pipx_available:
    ...
    command = f"{python_executable} -m mcp_code_intelligence.mcp.server {project_root}"

ELSE:
    # Fallback to system python
    command = f"python -m mcp_code_intelligence.mcp.server {project_root}"
...
**Files Modified**:
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/cli/commands/setup.py` (new)
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/cli/commands/mcp.py` (update)
- `/Users/masa/Projects/mcp-code-intelligence/docs/reference/MCP_SETUP.md` (update)

**Files Referenced**:
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/mcp/server.py` (MCP server entry point)
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/mcp/__main__.py` (Alternative entry point)
- `/Users/masa/Projects/mcp-code-intelligence/pyproject.toml` (Package configuration)
