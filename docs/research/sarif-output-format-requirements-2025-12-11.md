...
**Issue:** [#15 - Add SARIF Output Format](https://github.com/bobmatnyc/mcp-code-intelligence/issues/15)
...
This research report provides comprehensive requirements and implementation recommendations for adding SARIF (Static Analysis Results Interchange Format) 2.1.0 output to the mcp-code-intelligence code analysis tool. The implementation will enable seamless GitHub Code Scanning integration and CI/CD pipeline integration for automated code quality gates.
...
- `tool.driver.name`: Tool name (e.g., "MCP Code Intelligence")
...
```
src/mcp_code_intelligence/analysis/reporters/
...
class SARIFReporter:
    ...
    def __init__(
        self,
        tool_name: str = "MCP Code Intelligence",
        tool_version: str = "0.18.0",
        tool_uri: str = "https://github.com/bobmatnyc/mcp-code-intelligence",
    ) -> None:
...
**Update `src/mcp_code_intelligence/cli/commands/analyze.py`:**
...
**Create utility in `src/mcp_code_intelligence/utils/version.py`:**
...
def get_tool_version() -> str:
    ...
    try:
        return version("mcp-code-intelligence")
    except Exception:
        return "0.0.0-dev"
```
...
from mcp_code_intelligence.analysis.collectors.smells import CodeSmell, SmellSeverity
...
def test_generate_sarif_minimal():
    ...
    run = sarif["runs"][0]
    assert run["tool"]["driver"]["name"] == "MCP Code Intelligence"
...
1. Generate SARIF file for test project:
   ```bash
   mcp-code-intelligence analyze --format sarif --output results.sarif
   ```
  ...
  "runs": [
    {
      "tool": {
        "driver": {
          ...
          "version": "0.18.0",
          "informationUri": "https://github.com/bobmatnyc/mcp-code-intelligence",
          "rules": [
            {
              "id": "long-method",
              "shortDescription": {
          ...
          "locations": [
            {
              "physicalLocation": {
                "artifactLocation": {
                  "uri": "src/mcp_code_intelligence/core/indexer.py"
                },
                "region": {
                  "startLine": 145
                }
          ...
          "locations": [
            {
              "physicalLocation": {
                "artifactLocation": {
                  "uri": "src/mcp_code_intelligence/core/search.py"
                },
                "region": {
                  "startLine": 89
                }
          ...
          "locations": [
            {
              "physicalLocation": {
                "artifactLocation": {
                  "uri": "src/mcp_code_intelligence/parsers/base.py"
                },
                "region": {
                  "startLine": 1
                }
...
- [ ] Create `src/mcp_code_intelligence/analysis/reporters/sarif.py`
