...
## MCP Code Intelligence — `analyze` Command & Metrics Integration
...
Since mcp-code-intelligence already performs full AST traversal during indexing via Tree-sitter, computing structural metrics adds near-zero marginal cost. These metrics enable:
...
mcp-code-intelligence analyze [--output FORMAT] [--threshold-file FILE]
...
mcp-code-intelligence analyze --function "login_handler"

# Output formats
mcp-code-intelligence analyze --output json > metrics.json
mcp-code-intelligence analyze --output sarif > results.sarif  # For CI integration
mcp-code-intelligence analyze --output html > report.html

# Threshold configuration
mcp-code-intelligence analyze --fail-on-smell          # Exit code 1 if smells found
mcp-code-intelligence analyze --max-complexity 15      # Fail if any function exceeds
mcp-code-intelligence analyze --quality-gate strict    # Preset threshold profiles

# Diff-aware analysis (for CI/CD)
mcp-code-intelligence analyze --changed-only           # Only analyze git-changed files
mcp-code-intelligence analyze --baseline main          # Compare against branch

# Visualization export
mcp-code-intelligence analyze --export-viz ./viz-data/ # Export data for visualizer
```
...
mcp-code-intelligence search "database" --no-smells
mcp-code-intelligence search "api handler" --grade A,B

# Quality-aware ranking (default behavior)
mcp-code-intelligence search "login" --quality-weight 0.3  # 30% quality, 70% relevance
```
...
Run 'mcp-code-intelligence analyze --verbose' for detailed report
```
...
```yaml
# .mcp-code-intelligence/thresholds.yaml
