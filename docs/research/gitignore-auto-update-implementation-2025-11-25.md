# Research: Automatic .gitignore Updates for mcp-code-intelligence
...
This research analyzes how to safely implement automatic .gitignore updates during `mcp-code-intelligence init` to add the `.mcp-code-intelligence/` directory entry. The implementation should prioritize safety, handle edge cases gracefully, and follow best practices established in the codebase.

**Key Recommendations:**
1. Create new utility module: `src/mcp_code_intelligence/utils/gitignore_updater.py`
...
**File:** `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/core/project.py`
...
def initialize(self, ...) -> ProjectConfig:
    ...
    # 2. Create .mcp-code-intelligence directory
    index_path = get_default_index_path(self.project_root)  # Returns .mcp-code-intelligence/
...
**File:** `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/utils/gitignore.py`
...
**File:** `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/core/exceptions.py`
...
```gitignore
.mcp-code-intelligence/     # Directory pattern (preferred)
.mcp-code-intelligence      # Without trailing slash
.mcp-code-intelligence/*    # With wildcard
/.mcp-code-intelligence/    # Root-relative
```
...
5. Also check for negation patterns (`!.mcp-code-intelligence/`)
...
```gitignore
.*                      # Ignore all dotfiles
!.mcp-code-intelligence/    # But track this one
```
...
```gitignore
# MCP Code Intelligence index directory (auto-generated)
...
```gitignore
# MCP Code Intelligence index directory
.mcp-code-intelligence/
```
...
try:
    ...
    logger.info("Please manually add '.mcp-code-intelligence/' to your .gitignore")
...
if not gitignore_path.exists():
    ...
    content = "# MCP Code Intelligence index directory\n.mcp-code-intelligence/\n"
    gitignore_path.write_text(content, encoding="utf-8")
    logger.info("Created .gitignore with .mcp-code-intelligence/ entry")
    return
```
...
if not content:
    ...
    content = "# MCP Code Intelligence index directory\n.mcp-code-intelligence/\n"
    gitignore_path.write_text(content, encoding="utf-8")
    logger.info("Added .mcp-code-intelligence/ to empty .gitignore")
    return
```
...
```python
if ".mcp-code-intelligence/" in lines_normalized:
    logger.debug(".mcp-code-intelligence/ already in .gitignore")
    return
```
...
```python
if "!.mcp-code-intelligence" in content:
    logger.warning(
        ".gitignore contains negation pattern for .mcp-code-intelligence/. "
        "Your .mcp-code-intelligence/ directory may be tracked by git."
    )
...
```python
    ...
    logger.warning(
        ...
        "Please manually add '.mcp-code-intelligence/' to .gitignore"
    )
...
**Create:** `src/mcp_code_intelligence/utils/gitignore_updater.py`
...
def ensure_gitignore_entry(
    project_root: Path,
    pattern: str = ".mcp-code-intelligence/",
    comment: str | None = "MCP Code Intelligence index directory",
...
**File:** `src/mcp_code_intelligence/core/project.py`
...
```python
# src/mcp_code_intelligence/utils/gitignore_updater.py
...
```python
# src/mcp_code_intelligence/core/project.py
...
```python
subprocess.run(["git", "check-ignore", ".mcp-code-intelligence/"])
```
...
**File:** `src/mcp_code_intelligence/core/git_hooks.py`
...
result = subprocess.run(
    ["git", "check-ignore", ".mcp-code-intelligence/"],
    cwd=project_root,
    capture_output=True,
)
if result.returncode != 0:
    logger.warning(".mcp-code-intelligence/ is not ignored by git")
```
...
if not auto_gitignore:
    response = input("Add .mcp-code-intelligence/ to .gitignore? [Y/n]: ")
    if response.lower() != "y":
        return
```
...
**Files to Modify:**
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/core/project.py` (line 108)

**Files to Create:**
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/utils/gitignore_updater.py`
- `/Users/masa/Projects/mcp-code-intelligence/tests/unit/utils/test_gitignore_updater.py`
- `/Users/masa/Projects/mcp-code-intelligence/tests/integration/test_init_gitignore.py`

**Related Files (Reference Only):**
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/utils/gitignore.py` (existing parser)
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/core/git_hooks.py` (file modification pattern)
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/config/defaults.py` (constants)
