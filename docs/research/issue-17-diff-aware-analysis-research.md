...
**Issue**: [#17 - Diff-aware analysis](https://github.com/bobmatnyc/mcp-code-intelligence/issues/17)
...
Issue #17 implements **diff-aware analysis** capability for the `mcp-code-intelligence analyze` command, enabling CI/CD pipelines to analyze only changed files in a git repository. This feature reduces analysis time and provides focused quality feedback on code changes, making it ideal for pull request checks and continuous integration workflows.
...
- **Command Interface**: `mcp-code-intelligence analyze --changed-only`
- **Behavior**:
...
- **Command Interface**: `mcp-code-intelligence analyze --baseline <branch>`
- **Behavior**:
...
- **Examples**:
  ...
  mcp-code-intelligence analyze --baseline main --path src/core
  ```
...
**File**: `src/mcp_code_intelligence/core/git.py`
...
**Modifications to**: `src/mcp_code_intelligence/cli/commands/analyze.py`
...
- [ ] `mcp-code-intelligence analyze --changed-only` analyzes only uncommitted changes
...
- [ ] `mcp-code-intelligence analyze --baseline main` analyzes files differing from main branch
...
- Location: `src/mcp_code_intelligence/core/git.py`
...
import pytest
from pathlib import Path
from mcp_code_intelligence.core.git import GitManager, GitError
...
```bash
$ mcp-code-intelligence analyze --help
...
Usage Examples:
  ...
  # CI/CD quality gate
  $ mcp-code-intelligence analyze --baseline main --fail-on-smell --format sarif

  # Combined filters
  $ mcp-code-intelligence analyze --baseline main --language python --path src/
```
...
# CI/CD quality gate
mcp-code-intelligence analyze --baseline main --fail-on-smell
```
...
Use existing branch:
```bash
mcp-code-intelligence analyze --baseline master
```
...
### Internal Documents
- [Structural Code Analysis Design Document](./mcp-code-intelligence-structural-analysis-design.md)
