...
Phase 3 introduces **cross-file dependency analysis** and **persistent metrics storage** to mcp-code-intelligence. This phase shifts from single-file metrics (Phases 1-2) to project-wide relationship tracking, enabling:
...
**Integration Points**:
1. Extend `FileMetrics` in `src/mcp_code_intelligence/analysis/metrics.py`:
   ```python
   @dataclass
   class FileMetrics:
       # ... existing fields ...
...
2. Create `src/mcp_code_intelligence/analysis/collectors/coupling.py`

3. Register collector in `src/mcp_code_intelligence/core/indexer.py`
...
2. Add to `src/mcp_code_intelligence/analysis/collectors/coupling.py`
...
2. Create `src/mcp_code_intelligence/analysis/dependency_graph.py`
...
```python
# src/mcp_code_intelligence/analysis/storage/metrics_db.py
...
1. **Indexer Integration** (`src/mcp_code_intelligence/core/indexer.py`):
   ```python
...
2. **Analyze Command** (`src/mcp_code_intelligence/cli/commands/analyze.py`):
   ```python
   @click.command()
...
```python
# src/mcp_code_intelligence/analysis/trends.py
...
mcp-code-intelligence index --snapshot

# View recent trends
mcp-code-intelligence analyze --trends --days 30

# Compare against baseline
mcp-code-intelligence analyze --baseline v1.0.0

# Export trend data
mcp-code-intelligence analyze --trends --export json > trends.json
```
...
- [Structural Analysis Design](./mcp-code-intelligence-structural-analysis-design.md) - Full technical specification
...
Phase 3 introduces **persistent metrics storage** and **cross-file dependency analysis** to mcp-code-intelligence. The critical path starts with **Issue #24 (SQLite metrics store)**, which enables trend tracking and provides a foundation for all other Phase 3 features.
