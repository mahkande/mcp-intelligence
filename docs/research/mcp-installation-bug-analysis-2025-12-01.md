...
The `mcp-code-intelligence install claude-code` command has two critical bugs in the Claude CLI registration path:

1. **Server Name Bug**: Creates server named "mcp" instead of "mcp-code-intelligence"
...
**Fix Location**: `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/cli/commands/install.py`
...
mcp-code-intelligence: /Users/masa/.local/pipx/venvs/mcp-code-intelligence/bin/python -m mcp_code_intelligence.mcp.server /Users/masa/Projects/kuzu-memory
Status: ✓ Connected

# Broken server (Claude CLI):
mcp: uv run python -m mcp_code_intelligence.mcp.server /Users/masa/Projects/claude-mpm
Status: ✗ Failed to connect
```

### Issues
1. Server name is "mcp" (should be "mcp-code-intelligence")
2. Command is "uv run python ..." (should use actual Python path)
3. `uv run` fails because mcp-code-intelligence installed via pipx, not in uv project
...
### Bug Location
**File**: `src/mcp_code_intelligence/cli/commands/install.py`
...
cmd = [
    ...
    "--transport",
    "stdio",
    "mcp",  # Line 143 - BUG! Should be "mcp-code-intelligence"
    "--env",
    # ...
]
```
...
- mcp-code-intelligence is in a uv-managed project
...
def get_mcp_server_command(
    ...
    return f"{python_exe} -m mcp_code_intelligence.mcp.server{watch_flag} {project_root}"
```
...
- Changed server name from "mcp-code-intelligence" to "mcp"
...
> - **Server Name Change**: Changed from 'mcp-code-intelligence' to 'mcp' for consistency
...
### How Users Install mcp-code-intelligence

1. **Homebrew** (most common for Mac users)
   ```bash
   brew install mcp-code-intelligence
   # Creates: /opt/homebrew/Cellar/mcp-code-intelligence/X.Y.Z/libexec/bin/python3.11
   # Symlink: /opt/homebrew/bin/mcp-code-intelligence
   ```

2. **pipx** (recommended for Python users)
   ```bash
   pipx install mcp-code-intelligence
   # Creates: ~/.local/pipx/venvs/mcp-code-intelligence/bin/python
   # Symlink: ~/.local/bin/mcp-code-intelligence
   ```

3. **uv** (development/advanced users)
   ```bash
   uv run mcp-code-intelligence
...
# shutil.which("mcp-code-intelligence") = "/path/to/bin/mcp-code-intelligence"
...
def register_with_claude_cli(
    project_root: Path,
    server_name: str = "mcp-code-intelligence",  # ADD THIS PARAMETER
...
def register_with_claude_cli(
    ...
    Args:
        ...
        server_name: Name for the MCP server (default: "mcp-code-intelligence")
    ...
    try:
        ...
        if command != "mcp-code-intelligence":
            logger.warning(
...
def register_with_claude_cli(
    ...
    """Register MCP server with Claude CLI using native 'claude mcp add' command."""
    try:
        # Check if mcp-code-intelligence command is available
        if not shutil.which("mcp-code-intelligence"):
            logger.warning(
                "mcp-code-intelligence command not found in PATH, "
                "falling back to manual JSON configuration"
            )
            return False
        ...
        cmd = [
            ...
            f"MCP_ENABLE_FILE_WATCHING={'true' if enable_watch else 'false'}",
            "--",
            "mcp-code-intelligence",  # Use direct command
            "mcp",                # MCP subcommand
            str(project_root.absolute()),
        ]
...
- Uses `mcp-code-intelligence mcp` command directly
- Only works if `mcp-code-intelligence` is in PATH (pipx/homebrew installs)
...
# Verify
claude mcp list | grep "mcp-code-intelligence"
# Should show: mcp-code-intelligence with connected status

# Check command
cat .mcp.json | jq '.mcpServers["mcp-code-intelligence"]'
# Should use: "mcp-code-intelligence mcp" not "uv run"
```
...
```bash
# Setup
cd mcp-code-intelligence-repo
uv sync

# Test
uv run mcp-code-intelligence install claude-code

# Verify
cat .mcp.json
# Should use: "uv run mcp-code-intelligence mcp"
...
# Test: Re-run install
mcp-code-intelligence install claude-code --force
...
2. Run `mcp-code-intelligence install claude-code --force`
...
### Files to Change
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/cli/commands/install.py`
  - Function: `register_with_claude_cli()` (lines 103-175)
  - Function: `install_claude_code()` (line 587 caller)

### Files with Correct Reference Implementation
- `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/cli/commands/mcp.py`
...
- Change server name from "mcp" to "mcp-code-intelligence"
- Use `mcp-code-intelligence mcp` command directly
...
- Before: `claude mcp add mcp-code-intelligence ...`
- After: `claude mcp add mcp ...`

**Problem**: This broke consistency with:
- Manual JSON configs (use "mcp-code-intelligence")
- User expectations (package is "mcp-code-intelligence")
- Documentation (refers to "mcp-code-intelligence")
- Other MCP servers (use full package names)

**Recommendation**: Revert to "mcp-code-intelligence" for consistency across all installation methods.
...
After running `mcp-code-intelligence install claude-code` on a pipx install:

**Claude CLI State**:
```bash
$ claude mcp list
mcp-code-intelligence: mcp-code-intelligence mcp /path/to/project - ✓ Connected
```
