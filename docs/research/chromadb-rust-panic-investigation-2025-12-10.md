...
**Workaround**: Reset and rebuild the ChromaDB index (`mcp-code-intelligence reset && mcp-code-intelligence index`).
...
**Current Database**: `.mcp-code-intelligence/chroma.sqlite3`
...
**Affected Operations**:
- ✅ `mcp-code-intelligence search <query>` ← FAILS with panic
- ✅ `mcp-code-intelligence index` ← May trigger panic if index corrupted
...
### For mcp-code-intelligence Project
...
try:
    ...
    console.print("  mcp-code-intelligence reset && mcp-code-intelligence index")
    raise typer.Exit(1)
```
...
3. **Regular index validation**:
   ...
   mcp-code-intelligence reset && mcp-code-intelligence index
   ```
...
2. **Corruption Recovery**: `src/mcp_code_intelligence/core/database.py:653-722`
3. **Health Checks**: `src/mcp_code_intelligence/core/database.py:723-772`
...
for i in {1..10}; do
    mcp-code-intelligence index
    mcp-code-intelligence search "class" > /dev/null 2>&1
    mcp-code-intelligence reset --force
done

# If corruption occurred:
mcp-code-intelligence search "class"  # Should panic
```
...
# Test search
mcp-code-intelligence search "class definition"
...
**Bottom Line**: This is **not a bug in mcp-code-intelligence**, but rather a limitation in the upstream ChromaDB Rust bindings. Proper database lifecycle management and corruption recovery mechanisms mitigate the issue in production use.
