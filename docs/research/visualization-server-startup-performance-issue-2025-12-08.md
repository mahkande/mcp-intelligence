...
47817   02:50   0.0  uv run mcp-code-intelligence visualize serve --code-only --port 8088
```
...
# Start server
uv run mcp-code-intelligence visualize serve --code-only --port 8088 > /tmp/viz_test.log 2>&1 &
...
Modify `src/mcp_code_intelligence/cli/commands/visualize/graph_builder.py`:
...
```bash
# Check database size
ls -lh .mcp-code-intelligence/chroma.sqlite3

# Check for locks
lsof .mcp-code-intelligence/chroma.sqlite3

# Verify integrity
sqlite3 .mcp-code-intelligence/chroma.sqlite3 "PRAGMA integrity_check;"
```
...
```bash
# Run with profiling
python3 -m cProfile -o /tmp/profile.stats -m mcp_code_intelligence.cli.main visualize serve --code-only --port 8088
...
cd .mcp-code-intelligence/visualization
...
- Source code: `src/mcp_code_intelligence/cli/commands/visualize/graph_builder.py`
- Prescriptive layout: `src/mcp_code_intelligence/cli/commands/visualize/templates/scripts.py`
