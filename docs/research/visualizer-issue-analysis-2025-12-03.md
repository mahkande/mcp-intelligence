# Visualizer Issue Analysis - MCP Code Intelligence
...
The visualizer functionality is broken due to a mismatch between the code's expected file location and the actual location of visualization assets. Commit `cfabd24` (Nov 19, 2025) removed visualization files from `.mcp-code-intelligence/visualization/` directory, but the code in `visualize.py` still references a hardcoded path that expects `src/mcp_code_intelligence/visualization/` to exist.

**Impact**: `mcp-code-intelligence visualize serve` command fails silently because:
...
1. **`/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/cli/commands/visualize.py`** (1468 lines)
   - Main visualizer implementation
   - Exports chunk graph data to JSON
   - Serves D3.js visualization via HTTP server

2. **`/Users/masa/Projects/mcp-code-intelligence/scripts/test_visualization.py`** (309 lines)
   - Playwright-based test script
   - Tests visualization at http://localhost:8090
   - Validates graph rendering and node interactions

3. **`/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/cli/main.py`**
...
**Core Features:**
1. **Export Command** (`mcp-code-intelligence visualize export`)
...
2. **Serve Command** (`mcp-code-intelligence visualize serve`)
...
**Expected Path**: `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/visualization/`
...
- `__file__` = `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/cli/commands/visualize.py`
- `parent.parent.parent` = `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/`
- Result: `/Users/masa/Projects/mcp-code-intelligence/src/mcp_code_intelligence/visualization/`

**Installed Package** (via PyPI):
- `__file__` = `/usr/local/lib/python3.11/site-packages/mcp_code_intelligence/cli/commands/visualize.py`
- `parent.parent.parent` = `/usr/local/lib/python3.11/site-packages/mcp_code_intelligence/`
- Result: `/usr/local/lib/python3.11/site-packages/mcp_code_intelligence/visualization/`
...
1. **Commit 4f1cce0** (Oct 29, 2025): "refactor: move visualization files to .mcp-code-intelligence directory"
   - Moved visualization assets from source to `.mcp-code-intelligence/`
...
2. **Commit cfabd24** (Nov 19, 2025): "chore: remove obsolete visualization files"
   - Removed `index.html` from `.mcp-code-intelligence/visualization/`
...
Move visualization to project-local `.mcp-code-intelligence/visualization/` for persistence

**What commit cfabd24 did:**
Removed files from `.mcp-code-intelligence/visualization/` assuming they were obsolete

**What the code still expects:**
`src/mcp_code_intelligence/visualization/` (hardcoded path)

**Result**: Visualization directory doesn't exist at either location

### Why This Broke

The `visualize.py` code was **never updated** to reflect the move to `.mcp-code-intelligence/`. The hardcoded path at line 488 still points to the old location.
...
4f1cce0 - refactor: move visualization files to .mcp-code-intelligence directory (Oct 29, 2025)
089f717 - feat: comprehensive visualization enhancements (Oct 27, 2025)
```

**Breaking Commit**: `cfabd24` (Nov 19, 2025)
- **Author**: Bob Matsuoka
- **Changes**: Removed 25 files from `.mcp-code-intelligence/visualization/`
- **Impact**:
...
### 1. **`src/mcp_code_intelligence/cli/commands/visualize.py`** (HIGH PRIORITY)
...
```python
# Get project manager to use .mcp-code-intelligence directory
project_manager = ProjectManager(Path.cwd())
if not project_manager.is_initialized():
    console.print("[red]Project not initialized. Run 'mcp-code-intelligence init' first.[/red]")
    raise typer.Exit(1)

viz_dir = project_manager.project_root / ".mcp-code-intelligence" / "visualization"
```
...
- Already gitignored (`.mcp-code-intelligence/` is in `.gitignore`)
...
**Implementation**:
1. Create `src/mcp_code_intelligence/visualization/` directory
2. Add `index.html` as package data
3. Update `pyproject.toml`:
   ```toml
   [tool.setuptools.package-data]
   mcp_code_intelligence = ["visualization/*.html"]
   ```
4. Use `importlib.resources` to access files:
   ```python
   from importlib.resources import files
   viz_dir = files("mcp_code_intelligence") / "visualization"
   ```

**Files to Change**:
- `visualize.py`
- `pyproject.toml`
- New directory: `src/mcp_code_intelligence/visualization/`
...
**Location Examples**:
- Linux: `~/.local/share/mcp-code-intelligence/visualization/`
- macOS: `~/Library/Application Support/mcp-code-intelligence/visualization/`
- Windows: `%APPDATA%/mcp-code-intelligence/visualization/`
...
**File**: `src/mcp_code_intelligence/cli/commands/visualize.py`
...
try:
    ...
    if not project_manager.is_initialized():
        console.print(
            "[red]Project not initialized. Run 'mcp-code-intelligence init' first.[/red]"
        )
        raise typer.Exit(1)
...
2. Installed mode: `mcp-code-intelligence visualize serve`
3. Verify HTML generation in `.mcp-code-intelligence/visualization/`
...
The visualizer is broken due to a simple path mismatch between what the code expects (`src/mcp_code_intelligence/visualization/`) and where files were moved/deleted (`.mcp-code-intelligence/visualization/`).
...
4. **Documentation** (10 minutes):
   ...
   - Add note about `.mcp-code-intelligence/visualization/` location
