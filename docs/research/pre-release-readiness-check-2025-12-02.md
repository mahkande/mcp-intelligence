...
5. **src/mcp_code_intelligence/cli/commands/install.py** - Removed Claude Desktop installer
...
6. **src/mcp_code_intelligence/cli/commands/uninstall.py** - Removed Claude Desktop uninstaller
   - Removed `claude-desktop` from SUPPORTED_PLATFORMS dict
...
2. **Add BREAKING CHANGES section** documenting Claude Desktop removal:
   ```markdown
   ### BREAKING CHANGES
   - **Claude Desktop Support Removed**: The `mcp-code-intelligence install claude-desktop` and
     `mcp-code-intelligence uninstall claude-desktop` commands have been removed.
     ...
     - **Manual Cleanup**: Remove `mcp-code-intelligence` entry from `~/Library/Application Support/Claude/claude_desktop_config.json`
   ```
...
- src/mcp_code_intelligence/cli/commands/install.py: ✅ Removed
- src/mcp_code_intelligence/cli/commands/uninstall.py: ✅ Removed
...
3. **Stage Code Changes**:
   ```bash
   git add src/mcp_code_intelligence/cli/commands/install.py
   git add src/mcp_code_intelligence/cli/commands/uninstall.py
   ```
...
6. **Update Version**:
   ```bash
   # Edit src/mcp_code_intelligence/__init__.py
   # Change __version__ = "0.14.4" to __version__ = "0.15.0"
   git add src/mcp_code_intelligence/__init__.py
   ```
...
8. **Commit Changes**:
   ...
   Remove mcp-code-intelligence from ~/Library/Application Support/Claude/claude_desktop_config.json
...
1. **Installation Commands**:
   ```bash
   mcp-code-intelligence install list  # Should NOT show claude-desktop
   mcp-code-intelligence install claude-desktop  # Should return error or "command not found"
   ```

2. **Uninstallation Commands**:
   ```bash
   mcp-code-intelligence uninstall list  # Should NOT show claude-desktop
   mcp-code-intelligence uninstall claude-desktop  # Should return error or "command not found"
   ```

3. **Help Text**:
   ```bash
   mcp-code-intelligence install --help  # Should NOT mention claude-desktop
   mcp-code-intelligence uninstall --help  # Should NOT mention claude-desktop
   ```

4. **Setup Command**:
   ```bash
   mcp-code-intelligence setup  # Should NOT attempt Claude Desktop detection/configuration
   ```
...
grep -A20 "SUPPORTED_PLATFORMS" src/mcp_code_intelligence/cli/commands/install.py
grep -A20 "SUPPORTED_PLATFORMS" src/mcp_code_intelligence/cli/commands/uninstall.py
```
...
If you previously used `mcp-code-intelligence install claude-desktop`, follow these steps:
...
3. **Manual cleanup** (remove old Claude Desktop config):
   ...
   # Edit and remove mcp-code-intelligence server entry
   # File: ~/Library/Application Support/Claude/claude_desktop_config.json
   # Remove the "mcp-code-intelligence" or "mcp" entry from "mcpServers" section
   ```
