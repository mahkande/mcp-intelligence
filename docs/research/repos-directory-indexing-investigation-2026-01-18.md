# Investigation: mcp-code-intelligence repos/ Directory Indexing
...
The investigation revealed that `mcp-code-intelligence` is successfully indexing the `repos/` subdirectory in the deployment at `~/Clients/Duetto/CTO`. The perceived issue appears to be a misunderstanding rather than a technical problem.
...
$ grep -r "\brepos\b" /Users/masa/Projects/mcp-code-intelligence/src
src/mcp_code_intelligence/core/git.py:13:    - Git operations are typically fast (<100ms for most repos)
```

Only a comment reference found. No exclusion logic.

**Relevant Code Locations:**
- `src/mcp_code_intelligence/config/defaults.py` (lines 112-134): `DEFAULT_IGNORE_PATTERNS`
...
**Code Location:**
- `src/mcp_code_intelligence/utils/gitignore.py` (lines 58-68): Directory-only pattern matching
...
**Code Location:**
```python
# src/mcp_code_intelligence/core/indexer.py:1088-1146
...
mcp-code-intelligence init --file-extensions .py .js .ts .jsx .tsx .java .cpp .c .h .rs .go .rb .php .json .md .sh .yaml .yml
```

Or update `~/.config/mcp-code-intelligence/config.json`:
```json
{
  "file_extensions": [
...
```bash
cd ~/Clients/Duetto/CTO
mcp-code-intelligence search "forecasting" --limit 10
```
...
Enhance `mcp-code-intelligence status` to show directory-level statistics:

```bash
# Proposed enhancement
mcp-code-intelligence status --by-directory
...
**mcp-code-intelligence is working correctly** and indexing the repos/ subdirectory as expected. The perceived issue is likely due to:
...
- **DEFAULT_IGNORE_PATTERNS:** `src/mcp_code_intelligence/config/defaults.py:112-134`
- **Indexing logic:** `src/mcp_code_intelligence/core/indexer.py:954-1146`
- **Gitignore parsing:** `src/mcp_code_intelligence/utils/gitignore.py:1-248`
- **Batch processing:** `src/mcp_code_intelligence/core/indexer.py:586-696`
