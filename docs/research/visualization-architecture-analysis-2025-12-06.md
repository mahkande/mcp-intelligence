...
```python
# Core data model: CodeChunk (src/mcp_code_intelligence/core/models.py)
@dataclass
class CodeChunk:
...
1. **Directory Nodes**:
   - Loaded from `DirectoryIndex` (`.mcp-code-intelligence/directory_index.json`)
...
```
1. Indexing (mcp-code-intelligence index)
...
2. Directory Indexing (mcp-code-intelligence visualize)
   ├─ Scan project structure
   ├─ Build DirectoryIndex with metadata
   └─ Save to .mcp-code-intelligence/directory_index.json
...
4. Visualization Export (mcp-code-intelligence visualize export)
...
```
src/mcp_code_intelligence/cli/commands/visualize/
