...
The `mcp-code-intelligence chat` command uses AI to answer natural language questions about your code. It supports both **OpenAI** and **OpenRouter** APIs.
...
mcp-code-intelligence chat "where is the LLMClient class defined?"
...
mcp-code-intelligence chat "complex question" --model gpt-4o

# Use Claude 3.5 Sonnet via OpenRouter
mcp-code-intelligence chat "complex question" --model anthropic/claude-3.5-sonnet
```
...
mcp-code-intelligence chat "question" --json
```
...
API keys can also be stored in `~/.mcp-code-intelligence/config.json`:
...
2. Set environment variable or run `mcp-code-intelligence setup`
...
A: That's fine! The regular `mcp-code-intelligence search` command works without any API keys.
