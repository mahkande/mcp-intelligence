...
### Basic Usage
```bash
mcp-code-intelligence chat "where is X defined?"
```

### With Options
```bash
# Limit results
mcp-code-intelligence chat "find authentication code" --limit 3

# Use faster/cheaper model
mcp-code-intelligence chat "quick question" --model anthropic/claude-3-haiku

# Custom timeout
mcp-code-intelligence chat "complex question" --timeout 60

# JSON output
mcp-code-intelligence chat "find error handling" --json
```
...
📍 Result 1 of 5
📂 src/mcp_code_intelligence/core/search.py
...
```bash
$ mcp-code-intelligence chat "how does the search ranking algorithm work?"
...
```bash
$ mcp-code-intelligence chat "show me error handling patterns" --limit 3
...
#### 1. LLM Client (`src/mcp_code_intelligence/core/llm_client.py`)
- OpenRouter API integration
- Query generation
- Result analysis and ranking
- Error handling (timeouts, rate limits, invalid keys)

#### 2. Chat Command (`src/mcp_code_intelligence/cli/commands/chat.py`)
...
#### 3. Integration
- Registered in `src/mcp_code_intelligence/cli/main.py`
...
- Ensure code is indexed: `mcp-code-intelligence status`
- Try alternative phrasing

### Timeout Errors
```bash
# Increase timeout for complex queries
mcp-code-intelligence chat "query" --timeout 60
```
...
Part of MCP Code Intelligence project. See LICENSE file.
