...
All notable changes to MCP Code Intelligence will be documented in this file.
...
### Added
- **Zero-Config Setup Command**: New `mcp-code-intelligence setup` command for instant onboarding
  ...
  - **Consistent Server Naming**: MCP server registered as `mcp` (not `mcp-code-intelligence`) for consistency
  ...
  **Behind the scenes (MCP Integration):**
  ...
  - **Primary method**: `claude mcp add --transport stdio mcp --env MCP_ENABLE_FILE_WATCHING=true -- uv run python -m mcp_code_intelligence.mcp.server {PROJECT_ROOT}`
...
- **Enhanced Install Command with Native Claude CLI Support**: Updated `mcp-code-intelligence install` command with Claude CLI integration
  ...
  - **Consistent Naming**: Registers server as `mcp` (not `mcp-code-intelligence`)
...
- **Automatic .gitignore Entry Management**: Project initialization now automatically adds `.mcp-code-intelligence/` to `.gitignore`
  - Prevents accidental commits of local search index data to version control
  - Activates during `mcp-code-intelligence init` and `install` commands
  ...
  - New utility module: `src/mcp_code_intelligence/utils/gitignore_updater.py`
...
- **Indexing Configuration Options**: New configuration settings for fine-grained control over indexing behavior
  ...
  - Both settings can be configured via CLI: `mcp-code-intelligence config set <key> <value>`
...
- **MCP Command Reserved**: The `mcp` command is now exclusively for MCP server operations (not installation)
  - `mcp-code-intelligence mcp` - Start MCP server (stdio mode)
  - `mcp-code-intelligence mcp list` - List configured servers
  - `mcp-code-intelligence mcp test` - Test server startup
...
### Files Modified
- `src/mcp_code_intelligence/cli/commands/install.py` - Rewritten with hierarchical structure
- `src/mcp_code_intelligence/cli/commands/uninstall.py` - New command for MCP removal
- `src/mcp_code_intelligence/cli/main.py` - Updated command registration and help text
...
```bash
mcp-code-intelligence init --mcp
```
...
- Expanded ignore patterns: `.mypy_cache`, `.ruff_cache`, `.claude-mpm`, `.mcp-code-intelligence`
...
### Files Modified
- `src/mcp_code_intelligence/visualization/index.html` - UI improvements, expand/collapse fixes, cache prevention
- `src/mcp_code_intelligence/cli/commands/visualize.py` - Parent directory path normalization and linking fixes
...
### 🎯 Impact
This release fixes the core search functionality that was completely broken in v0.4.0, making MCP Code Intelligence fully functional and production-ready for real-world use cases.
...
This is the first public release of MCP Code Intelligence! We're excited to share this semantic code search tool with the developer community.
...
**Feedback Welcome:**
This is an alpha release - we're actively seeking feedback on search quality, performance, and usability. Please [open an issue](https://github.com/bobmatnyc/mcp-code-intelligence/issues) or start a [discussion](https://github.com/bobmatnyc/mcp-code-intelligence/discussions)!
