...
MCP Code Intelligence follows a structured release process that ensures quality, reliability, and clear communication with users. Our release strategy balances rapid iteration with stability guarantees.
...
#### Update Version
```python
# src/mcp_code_intelligence/__init__.py
__version__ = "0.0.4"
```
...
git add src/mcp_code_intelligence/__init__.py docs/CHANGELOG.md
...
pip install mcp-code-intelligence==0.0.4 --upgrade
...
pip install mcp-code-intelligence==0.0.4
mcp-code-intelligence version
```
...
- **New Language Support**: Added Rust parsing with support for `impl` blocks and `trait` definitions
  ```bash
  mcp-code-intelligence index --language rust
  ```
...
pypistats recent mcp-code-intelligence

# GitHub metrics
gh api repos/bobmatnyc/mcp-code-intelligence \
  --jq '.stargazers_count, .forks_count, .open_issues_count'

# Release-specific metrics
gh api repos/bobmatnyc/mcp-code-intelligence/releases/latest \
