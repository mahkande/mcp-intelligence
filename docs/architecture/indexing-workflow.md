...
This document provides a comprehensive analysis of how reindexing works in the MCP Code Intelligence system, based on extensive testing and code analysis.
...
- Stores metadata in `.mcp-code-intelligence/index_metadata.json`
...
1. **Use incremental indexing for regular updates**
   ```bash
   mcp-code-intelligence index  # Incremental by default
   ```

2. **Use force reindexing periodically**
   ```bash
   mcp-code-intelligence index --force  # Full reindex
   ```

3. **Use file watching for real-time updates**
   ```bash
   mcp-code-intelligence watch  # Real-time monitoring
   ```
