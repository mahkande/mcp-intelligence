...
```bash
mcp-code-intelligence search "function that validates user input"
mcp-code-intelligence search "database connection with retry logic"
...
```bash
mcp-code-intelligence index                    # Index entire project
mcp-code-intelligence index --force            # Force full reindex
mcp-code-intelligence index /path/to/specific  # Index specific directory
```
...
```bash
mcp-code-intelligence watch        # Start file watching
mcp-code-intelligence watch status # Check watching status
```
...
```bash
mcp-code-intelligence auto-index check --auto-reindex --max-files 10
...
```python
from mcp_code_intelligence.core.auto_indexer import PeriodicIndexChecker
