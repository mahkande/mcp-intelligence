...
Implemented a three-layer protection strategy to prevent ChromaDB crashes when running `mcp-code-intelligence status` on large databases.
...
**File**: `src/mcp_code_intelligence/cli/commands/status.py`

Added two new flags to the `status` command:

```bash
# Skip statistics collection (safe mode)
mcp-code-intelligence status --skip-stats

# Force full statistics (may crash on large DBs)
mcp-code-intelligence status --force-stats
```
...
**File**: `src/mcp_code_intelligence/core/models.py`
...
mcp-code-intelligence status --json
...
- MCP Code Intelligence Issue: https://github.com/.../issues/...
